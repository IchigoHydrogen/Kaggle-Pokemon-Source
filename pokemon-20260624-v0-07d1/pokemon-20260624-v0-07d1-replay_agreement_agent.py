...
# ============================================================
# v0-07d1 Guarded MAIN option scorer — appended to rule agent
# Changes vs v06d18:
#   - RL-trained via PPO from IL-initialized policy
#   - Guarded: model CAN override rule agent within SAFE_OVERRIDE_TYPES
# ============================================================
    ...
    class _V06D15MainScorer(_v06d15_nn.Module):
        def __init__(self):
            ...
            self.net = _v06d15_nn.Sequential(
                _v06d15_nn.Linear(97, 512), _v06d15_nn.ReLU(),
...
def _v06d15_features_batch(obs, opts):
    ...
    n = len(opts)
    if n == 0:
        return _v06d15_np.zeros((0, 97), dtype=_v06d15_np.float32)
    X = _v06d15_np.zeros((n, 97), dtype=_v06d15_np.float32)
    cur = obs.get('current') or {}
    sel = obs.get('select') or {}
    ...
    for _oi in range(n):
        ...
        X[_oi, 61:96] = _meta
    X[:, 96] = 1.0  # return-conditioned: always win
    return X


_V06D15_SAFE_OVERRIDE_TYPES = frozenset(range(16))  # guarded: all MAIN option types
_V06D15_HARD_VETO_TYPES = frozenset()  # no hard vetoes in guarded mode
_V06D15_PROB_THRESHOLD = 0.35
_V06D15_MARGIN_THRESHOLD = 0.0
...
def agent(obs_dict: dict) -> list:
    ...
    try:
        ...
        if top1 != rule_idx:
            ...
            if (top1_type not in _V06D15_HARD_VETO_TYPES and
                    top1_type in _V06D15_SAFE_OVERRIDE_TYPES):
                _exps = [_v06d15_np.exp(float(_l) - float(_v06d15_np.max(logits)))
                         for _l in logits]
                _esum = float(sum(_exps))
                _top1_prob = _exps[top1] / _esum if _esum > 0 else 0.0
                if _top1_prob >= _V06D15_PROB_THRESHOLD:
                    _v06d15_action_changes += 1
                    return [top1]
        return rule_result
    except Exception:
        return rule_result
