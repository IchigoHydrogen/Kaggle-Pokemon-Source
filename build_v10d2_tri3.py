...
WD = float(sys.argv[5]) if len(sys.argv) > 5 else 0.5
...
def _dev_eval(obs, p):
    ...
    alak = 1 if (my_act_id == 743 or 743 in ids) else 0
    my_bench = len(me.get('bench') or []); my_deck = me.get('deckCount', 0)
    my_hand = me.get('handCount', 0) or len(me.get('hand') or [])
    return 40.0*alak + 8.0*my_e + 4.0*my_bench + 3.0*my_hand - 60.0*(1 if my_deck <= 3 else 0)
...
def agent(obs_dict):
    ...
    try:
        ...
        for oi in range(n):
            ...
            dv.append(_dev_eval(end, p) if end else -1e17)
        try: _rapi.search_end()
        except Exception: pass
        rh, rg, rd = _ranks(hv), _ranks(gv), _ranks(dv)
        combined = [rh[i] + _DUAL_W * rg[i] + _DUAL_WD * rd[i] for i in range(n)]
...
print('compiles OK (W=%s WD=%s)' % (W, WD))
