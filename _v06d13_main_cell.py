# =====================================================================
# v0-06d13 Part A: Rule-family conditioned ATTACK/END diagnostics
#
...
# =====================================================================
import importlib.util as _v06d13_ilu
import tarfile as _v06d13_tarfile
import time as _v06d13_time
import torch.nn.functional as _v06d13_F
from collections import defaultdict as _v06d13_dd
...
_V06D13_CARD_META_OFFSET = 61  # start index in 96-dim vector
_V06D13_FEAT_DIM = 96

def _v06d13_name(mapping, value, prefix):
    try:
        return mapping.get(int(value), prefix + '_' + str(value))
    except Exception:
        return prefix + '_' + str(value)

def _v06d13_norm_name(s):
    return re.sub(r'[^a-z0-9]', '', str(s).lower())

def _v06d13_rank_bucket(rank):
    if rank is None:
        return 'unranked'
    if rank <= 10:
        return 'top10'
...
def _v06d13_archetype_of_deck(deck):
...
def _v06d13_load_agent_from_archive(archive_path):
    with _v06d13_tarfile.open(archive_path) as t:
    ...
    spec = _v06d13_ilu.spec_from_file_location('v06d13_main_learning_rule_agent', str(mod_path))
    m = _v06d13_ilu.module_from_spec(spec)
...
def _v06d13_card_meta_vec(card_id, card_table):
    """Return 35-dim card metadata vector for card_id."""
    vec = [0.0] * _V06D13_CARD_META_DIM
...
def _v06d13_option_features(obs, option, option_index, option_count, card_table):
    ...
    card_id = _v06d13_card_id_from_area(obs, option)
    row = [0.0] * _V06D13_FEAT_DIM
    # [0-12]: global context
    row[0] = 1.0
    row[1] = min(1.0, option_count / 32.0)
    row[2] = min(1.0, option_index / max(1, option_count - 1))
    row[3] = _v06d13_to_float(select.get('minCount'), 0.0) / 8.0
    row[4] = _v06d13_to_float(select.get('maxCount'), 0.0) / 8.0
    row[5] = min(1.0, _v06d13_to_float(cur.get('turn'), 0.0) / 20.0)
    row[6] = min(1.0, _v06d13_to_float(cur.get('turnActionCount'), 0.0) / 32.0)
    row[7] = min(1.0, _v06d13_to_float(obs.get('step'), 0.0) / 512.0)
    row[8] = float(your)
    ...
    row[27] = (area % 16) / 16.0
    row[28] = _v06d13_to_float(option.get('index'), 0.0) / 32.0
    row[29] = _v06d13_to_float(option.get('inPlayIndex'), 0.0) / 16.0
    row[30] = _v06d13_to_float(option.get('number'), 0.0) / 16.0
    row[31] = _v06d13_to_float(option.get('attackId'), 0.0) / 4096.0
    row[32] = _v06d13_to_float(option.get('abilityId'), 0.0) / 4096.0
    # [33-48]: opt_type one-hot (16 dims)
    row[33 + (opt_type % 16)] = 1.0
    # [49-60]: area one-hot (12 dims)
    row[49 + (area % 12)] = 1.0
    # [61-95]: card metadata (35 dims)
    meta = _v06d13_card_meta_vec(card_id, card_table)
    for mi, mv in enumerate(meta):
        row[_V06D13_CARD_META_OFFSET + mi] = mv
    return row
...
try:
    ...
    _rule_mod, _our_deck = _v06d13_load_agent_from_archive(_archive)
    _rule_agent = _rule_mod.agent
    _our_arch = _v06d13_archetype_of_deck(_our_deck)

    # Load card metadata
    _t0_card = _v06d13_time.perf_counter()
    from cg.api import all_card_data as _v06d13_all_card_data
    _card_table = {c.cardId: c for c in _v06d13_all_card_data()}
    print(f'card table loaded: {len(_card_table)} cards in {_v06d13_time.perf_counter()-_t0_card:.2f}s')
    ...
    def _rank_of(name):
        info = _rank_info.get(_v06d13_norm_name(name), {}) if isinstance(_rank_info, dict) else {}
        r = info.get('ranking') if isinstance(info, dict) else None
        return int(r) if r is not None else None

    _max_decisions = int(os.environ.get('V06D13_MAIN_MAX_DECISIONS', '120000'))
    _max_options = int(os.environ.get('V06D13_MAIN_MAX_OPTIONS', '1600000'))
    ...
    # ------------------------------------------------------------------
    _t0_extract = _v06d13_time.perf_counter()
    _alloc_size = _max_options + 1024
    _X = np.zeros((_alloc_size, _V06D13_FEAT_DIM), dtype=np.float32)
    _y = np.zeros(_alloc_size, dtype=np.float32)
    _cursor = 0
    _decision_rows = []
    _option_meta_rows = []
    _decision_id = 0
    _stats = _v06d13_dd(int)
    _cross_check_buf = []
    ...
    for _cid in _known_card_ids:
        _card_meta_lookup[_cid] = np.array(_v06d13_card_meta_vec(_cid, _card_table), dtype=np.float32)
    _zero_meta = np.zeros(_V06D13_CARD_META_DIM, dtype=np.float32)
    ...
    for _fp in EPISODE_FILES:
        ...
        _seat_arch = {k: _v06d13_archetype_of_deck(v) for k, v in _seat_deck.items()}
        ...
        for _s in _steps:
            ...
            for _seat, _a in enumerate(_s):
                ...
                _opp_idx = 1 - _your
                _mine = _v06d13_player_summary(_obs, _your)
                _theirs = _v06d13_player_summary(_obs, _opp_idx)
                _context = int(_sel2.get('context', 0) or 0)

                # Shared decision-level scalars
                _f_bias = 1.0
                _f_opt_count = min(1.0, _nopt / 32.0)
                _f_min_count = _v06d13_to_float(_sel2.get('minCount'), 0.0) / 8.0
                _f_max_count = _v06d13_to_float(_sel2.get('maxCount'), 0.0) / 8.0
                _f_turn = min(1.0, _v06d13_to_float(_cur.get('turn'), 0.0) / 20.0)
                _f_turn_act = min(1.0, _v06d13_to_float(_cur.get('turnActionCount'), 0.0) / 32.0)
                _f_step = min(1.0, _v06d13_to_float(_obs.get('step'), 0.0) / 512.0)
                _f_your = float(_your)
                ...
                _card_ids = np.array([_v06d13_card_id_from_area(_obs, _o if isinstance(_o, dict) else {}) for _o in _opts], dtype=np.int32)
                _opt_indices = np.array([_v06d13_to_float((_o or {}).get('index'), 0.0) for _o in _opts], dtype=np.float32)
                _in_plays = np.array([_v06d13_to_float((_o or {}).get('inPlayIndex'), 0.0) for _o in _opts], dtype=np.float32)
                _numbers = np.array([_v06d13_to_float((_o or {}).get('number'), 0.0) for _o in _opts], dtype=np.float32)
                _attack_ids = np.array([_v06d13_to_float((_o or {}).get('attackId'), 0.0) for _o in _opts], dtype=np.float32)
                _ability_ids = np.array([_v06d13_to_float((_o or {}).get('abilityId'), 0.0) for _o in _opts], dtype=np.float32)
                ...
                for _oi in range(_nopt):
                    ...
                    _X[_s_row + _oi, _V06D13_CARD_META_OFFSET:_V06D13_CARD_META_OFFSET + _V06D13_CARD_META_DIM] = _meta
                ...
                for _oi, _opt in enumerate(_opts):
                    _option_meta_rows.append({
                        ...
                        'option_type_name': _v06d13_name(_V06D13_OPTION_NAMES, int((_opt or {}).get('type', -1)) if isinstance(_opt, dict) else -1, 'OPT'),
                ...
                _decision_rows.append({
                    ...
                    'chosen_option_type_name': _v06d13_name(_V06D13_OPTION_NAMES, int(_chosen_opt.get('type', -1)), 'OPT'),
                    'rule_index': int(_rule_idx),
                    'rule_hit': int(_rule_idx == _act[0]),
                    'rank': _rank,
                    'rank_bucket': _v06d13_rank_bucket(_rank),
                    'won': bool(_won),
    ...
    _X = _X[:_cursor]
    _y = _y[:_cursor]
    _t1_extract = _v06d13_time.perf_counter()
    _extract_time_sec = _t1_extract - _t0_extract
    print(f'v0-06d13 extraction: {_extract_time_sec:.1f}s, {_cursor} option rows, {_decision_id} decisions')
    ...
    for _cc_obs, _cc_opts, _cc_nopt, _cc_start in _cross_check_buf:
        for _cc_oi, _cc_opt in enumerate(_cc_opts):
            _ref = _v06d13_option_features(_cc_obs, _cc_opt if isinstance(_cc_opt, dict) else {}, _cc_oi, _cc_nopt, _card_table)
    ...
    ARTIFACT_PATHS['v06d13_main_feature_matrix'] = str(_feat_npy_path)
    ARTIFACT_PATHS['v06d13_main_labels'] = str(_label_npy_path)

    _feature_spec = {
        'feature_dim': _V06D13_FEAT_DIM,
        'layout_version': 'v06d13',
        'card_id_hash_encoding': False,
        'card_metadata_encoding': True,
        'card_meta_dim': _V06D13_CARD_META_DIM,
        'card_meta_offset': _V06D13_CARD_META_OFFSET,
        'target': 'recorded replay MAIN option index for alakazam actor',
        'loss': 'per_decision_cross_entropy',
        'extraction_method': 'numpy_preallocated_vectorized_v06d13',
    ...
    ARTIFACT_PATHS['v06d13_main_feature_spec'] = write_json(OUTPUT_DIR / 'main_feature_spec.json', _feature_spec)
    ...
    _dataset_report = {
        ...
        'feature_dim': int(_V06D13_FEAT_DIM),
    ...
    ARTIFACT_PATHS['v06d13_main_dataset_report'] = write_json(OUTPUT_DIR / 'main_dataset_report.json', _dataset_report)
    ARTIFACT_PATHS['v06d13_main_decision_rows'] = safe_save_table(_decision_df, OUTPUT_DIR / 'main_decision_rows.parquet')
    ...
    # ------------------------------------------------------------------
    class _V06D13MainScorer(nn.Module):
    ...
    _model = _V06D13MainScorer(_V06D13_FEAT_DIM).to(_device)
    _param_count = int(sum(p.numel() for p in _model.parameters()))
    print(f'model: {_param_count} params, device={_device}')

    _max_epochs = int(os.environ.get('V06D13_MAIN_EPOCHS', '100'))
    _patience = int(os.environ.get('V06D13_MAIN_PATIENCE', '5'))
    _decision_batch = int(os.environ.get('V06D13_DECISION_BATCH', '256'))
    _lr = float(os.environ.get('V06D13_MAIN_LR', '0.002'))
    ...
    if _SKIP_TRAIN:
        ...
        _t0_gpu = _v06d13_time.perf_counter()
        try:
        ...
        for _ep in range(_max_epochs):
            ...
            for _st in range(0, _n_tr, _decision_batch):
                ...
                _xb_np = np.zeros((_B, _max_n, _V06D13_FEAT_DIM), dtype=np.float32)
                ...
                _logits = _model(_xb.view(_B * _max_n, _V06D13_FEAT_DIM)).view(_B, _max_n)
        ...
        _model.eval()
        _train_time_sec = _v06d13_time.perf_counter() - _t0_train

        # Save model
        torch.save(_model.state_dict(), _model_path)
    ARTIFACT_PATHS['v06d13_main_option_scorer'] = str(_model_path)

    # ------------------------------------------------------------------
    # Inference on full dataset (flat row-by-row for prediction table)
    # ------------------------------------------------------------------
    _t0_infer = _v06d13_time.perf_counter()
    ...
    _infer_time_sec = _v06d13_time.perf_counter() - _t0_infer
    ...
    ARTIFACT_PATHS['v06d13_main_holdout_predictions'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d13_main_eval_by_bucket'] = safe_save_table(_eval_df, OUTPUT_DIR / 'main_eval_by_bucket.parquet')

    # ------------------------------------------------------------------
    # Hybrid evaluation — keep v06d12 safe-family guard as a comparison baseline.
    # v06d13's main evidence is the conditioned ATTACK/END diagnostics below.
    # Threshold grid on valid; holdout only for promotion check.
    # ------------------------------------------------------------------
    _safe_override_types = [x.strip().upper() for x in
        os.environ.get('V06D13_SAFE_MAIN_OVERRIDE_TYPES', 'PLAY,ATTACH,EVOLVE,RETREAT').split(',') if x.strip()]
    _veto_types = ['ABILITY']  # only hard veto remaining
    _prob_grid = [float(x) for x in
        os.environ.get('V06D13_HYBRID_PROB_GRID', '0.35,0.45,0.55,0.65,0.75,0.85').split(',') if str(x).strip()]
    _margin_grid = [float(x) for x in
        os.environ.get('V06D13_HYBRID_MARGIN_GRID', '0.00,0.10,0.20,0.35,0.50').split(',') if str(x).strip()]
    ...
    ARTIFACT_PATHS['v06d13_main_hybrid_threshold_grid'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d13_main_hybrid_eval_by_bucket'] = safe_save_table(
        _hybrid_bucket_df, OUTPUT_DIR / 'main_hybrid_eval_by_bucket.parquet')

    # ------------------------------------------------------------------
    # v06d13 diagnostics: condition evaluation on the rule-selected option family.
    ...
    ARTIFACT_PATHS['v06d13_rule_option_type_conditioned_report'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d13_attack_within_type_threshold_grid'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d13_attack_within_type_rerank_report'] = write_json(
    ...
    ARTIFACT_PATHS['v06d13_end_deferral_threshold_grid'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d13_end_deferral_report'] = write_json(
    ...
    ARTIFACT_PATHS['v06d13_cross_family_contamination_report'] = write_json(
    ...
    MAIN_HYBRID_REPORT = {
        'status': 'ok',
        'version': RUN_PREFIX,
        'purpose': 'per-decision CE + card metadata features + rule-option-type conditioned ATTACK/END diagnostics',
        'loss': 'per_decision_cross_entropy',
        'features': 'card_metadata_v06d13',
        'guard_scope': 'diagnostic only for ATTACK/END; safe-family hybrid retained as comparison baseline',
    ...
    MAIN_LEARNING_REPORT = MAIN_HYBRID_REPORT
    ARTIFACT_PATHS['v06d13_main_model_report'] = write_json(OUTPUT_DIR / 'main_model_report.json', MAIN_HYBRID_REPORT)
    ARTIFACT_PATHS['v06d13_main_hybrid_report'] = write_json(OUTPUT_DIR / 'main_hybrid_report.json', MAIN_HYBRID_REPORT)

    print('v0-06d13 Part A status: ok')
...
except Exception as _v06d13_exc:
    MAIN_HYBRID_REPORT = {'status': 'error', 'error': repr(_v06d13_exc), 'traceback': traceback.format_exc(limit=8)}
    MAIN_LEARNING_REPORT = MAIN_HYBRID_REPORT
    ARTIFACT_PATHS['v06d13_main_model_report'] = write_json(OUTPUT_DIR / 'main_model_report.json', MAIN_HYBRID_REPORT)
    print('v0-06d13 Part A FAILED:', repr(_v06d13_exc))
    raise
