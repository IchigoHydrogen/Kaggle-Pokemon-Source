# =====================================================================
# v0-06d15 Part B: Shadow runtime probe + Kaggle loader entrypoint check
...
# Feature extractor updated to v06d15 layout (card metadata, no card_id hash).
# Model output is measured but never changes selected actions in v06d15.
# =====================================================================
import shutil as _v06d15b_shutil
import tarfile as _v06d15b_tarfile
...
try:
    ...
    _v06d15b_shutil.copy2(str(_model_pt_src), str(_local_model_dst))
    ...
    _GUARD_INJECTION = f'''
# ============================================================
# v0-06d15 Shadow MAIN option scorer — appended to rule agent
# Changes vs v06d11:
#   - Feature layout v06d15: card metadata [61-95], no card_id hash
#   - Shadow only: model output never changes selected actions
# ============================================================
_v06d15_rule_agent_fn = agent  # save original

_V06D15_MODEL_LOADED = False
_v06d15_model = None

try:
    import numpy as _v06d15_np
    import time as _v06d15_time
    import torch as _v06d15_torch
    import torch.nn as _v06d15_nn
    import sys as _v06d15_sys
    from pathlib import Path as _v06d15_Path

    class _V06D15MainScorer(_v06d15_nn.Module):
        def __init__(self):
            super().__init__()
            self.net = _v06d15_nn.Sequential(
                _v06d15_nn.Linear(96, 512), _v06d15_nn.ReLU(),
                _v06d15_nn.Linear(512, 384), _v06d15_nn.ReLU(),
                _v06d15_nn.Linear(384, 256), _v06d15_nn.ReLU(),
                _v06d15_nn.Linear(256, 1),
            )
        def forward(self, x):
            return self.net(x).squeeze(-1)

    _v06d15_device = _v06d15_torch.device(\'cpu\')
    _v06d15_base_dir = _v06d15_Path(globals().get(\'__file__\', _v06d15_sys.path[-1])).parent
    if not (_v06d15_base_dir / \'models\' / \'main_option_scorer.pt\').exists():
        _v06d15_base_dir = _v06d15_Path(_v06d15_sys.path[-1])
    _v06d15_model_path = _v06d15_base_dir / \'models\' / \'main_option_scorer.pt\'
    _v06d15_model = _V06D15MainScorer().to(_v06d15_device)
    _v06d15_model.load_state_dict(
        _v06d15_torch.load(str(_v06d15_model_path), map_location=\'cpu\', weights_only=True)
    )
    _v06d15_model.eval()

    # Build card metadata lookup from bundled cg package
    _V06D15_CARD_META_DIM = 35
    _V06D15_CARD_META_OFFSET = 61
    _v06d15_card_meta_lookup = {{}}
    try:
        if str(_v06d15_base_dir) not in _v06d15_sys.path:
            _v06d15_sys.path.insert(0, str(_v06d15_base_dir))
        from cg.api import all_card_data as _v06d15_all_card_data
        for _c in _v06d15_all_card_data():
            _cid = int(_c.cardId)
            _vec = [0.0] * _V06D15_CARD_META_DIM
            ...
            _v06d15_card_meta_lookup[_cid] = _v06d15_np.array(_vec, dtype=_v06d15_np.float32)
    except Exception:
        pass  # card metadata dims 0-15 will be zero; key-card flags built inline

    _V06D15_ZERO_META = _v06d15_np.zeros(_V06D15_CARD_META_DIM, dtype=_v06d15_np.float32)

    _V06D15_MODEL_LOADED = True
except Exception as _v06d15_load_err:
    _V06D15_MODEL_LOADED = False
    _v06d15_load_err_msg = repr(_v06d15_load_err)
...
def _v06d15_features_batch(obs, opts):
    """Extract (n_opts, 96) feature matrix — v06d15 layout (card metadata, no card_id hash)."""
    n = len(opts)
    if n == 0:
        return _v06d15_np.zeros((0, 96), dtype=_v06d15_np.float32)
    X = _v06d15_np.zeros((n, 96), dtype=_v06d15_np.float32)
    cur = obs.get(\'current\') or {{}}
    sel = obs.get(\'select\') or {{}}
    ...
    X[:, 0] = 1.0
    X[:, 1] = min(1.0, n / 32.0)
    X[:, 2] = _v06d15_np.minimum(1.0, _v06d15_np.arange(n, dtype=_v06d15_np.float32) / denom)
    X[:, 3] = _v06d15_to_float(sel.get(\'minCount\'), 0.0) / 8.0
    X[:, 4] = _v06d15_to_float(sel.get(\'maxCount\'), 0.0) / 8.0
    X[:, 5] = min(1.0, _v06d15_to_float(cur.get(\'turn\'), 0.0) / 20.0)
    X[:, 6] = min(1.0, _v06d15_to_float(cur.get(\'turnActionCount\'), 0.0) / 32.0)
    X[:, 7] = min(1.0, _v06d15_to_float(obs.get(\'step\'), 0.0) / 512.0)
    X[:, 8] = float(your)
    ...
    opt_types = _v06d15_np.array([int((o or {{}}).get(\'type\', 0) or 0) for o in opts], dtype=_v06d15_np.int32)
    areas = _v06d15_np.array([int((o or {{}}).get(\'area\', 0) or 0) for o in opts], dtype=_v06d15_np.int32)
    card_ids = _v06d15_np.array([
        _v06d15_card_id_from_area(obs, o if isinstance(o, dict) else {{}}) for o in opts
    ], dtype=_v06d15_np.int32)
    opt_indices = _v06d15_np.array([_v06d15_to_float((o or {{}}).get(\'index\'), 0.0) for o in opts], dtype=_v06d15_np.float32)
    in_plays = _v06d15_np.array([_v06d15_to_float((o or {{}}).get(\'inPlayIndex\'), 0.0) for o in opts], dtype=_v06d15_np.float32)
    numbers = _v06d15_np.array([_v06d15_to_float((o or {{}}).get(\'number\'), 0.0) for o in opts], dtype=_v06d15_np.float32)
    attack_ids = _v06d15_np.array([_v06d15_to_float((o or {{}}).get(\'attackId\'), 0.0) for o in opts], dtype=_v06d15_np.float32)
    ability_ids = _v06d15_np.array([_v06d15_to_float((o or {{}}).get(\'abilityId\'), 0.0) for o in opts], dtype=_v06d15_np.float32)
    # [26-32]: option numerics (v06d15 layout — no card_id continuous)
    ...
    for _oi in range(n):
        ...
        _meta = _v06d15_card_meta_lookup.get(_cid, _V06D15_ZERO_META)
        X[_oi, 61:96] = _meta
    return X


_V06D15_SAFE_OVERRIDE_TYPES = frozenset()  # shadow only
_V06D15_HARD_VETO_TYPES = frozenset()      # shadow only
_V06D15_PROB_THRESHOLD = {_sel_prob_b}
_V06D15_MARGIN_THRESHOLD = {_sel_margin_b}

_v06d15_action_changes = 0
_v06d15_shadow_disagreements = 0
_v06d15_shadow_type_counts = {{}}
_v06d15_decisions_seen = 0
_v06d15_latencies_ms = []


def agent(obs_dict: dict) -> list:
    global _v06d15_action_changes, _v06d15_shadow_disagreements, _v06d15_shadow_type_counts
    global _v06d15_decisions_seen, _v06d15_latencies_ms
    rule_result = _v06d15_rule_agent_fn(obs_dict)
    if not _V06D15_MODEL_LOADED:
        return rule_result
    try:
        ...
        if rule_type in _V06D15_HARD_VETO_TYPES:
            return rule_result
        t0 = _v06d15_time.perf_counter()
        X = _v06d15_features_batch(obs_dict, opts)
        with _v06d15_torch.no_grad():
            logits = _v06d15_model(
                _v06d15_torch.tensor(X, dtype=_v06d15_torch.float32)
            ).numpy()
        _v06d15_latencies_ms.append((_v06d15_time.perf_counter() - t0) * 1000.0)
        _v06d15_decisions_seen += 1
        top1 = int(_v06d15_np.argmax(logits))
        top1_opt = opts[top1] if (0 <= top1 < n and isinstance(opts[top1], dict)) else {{}}
        top1_type = int(top1_opt.get(\'type\', -1))
        if top1 != rule_idx:
            _v06d15_shadow_disagreements += 1
            _v06d15_shadow_type_counts[top1_type] = _v06d15_shadow_type_counts.get(top1_type, 0) + 1
        return rule_result
    except Exception:
        return rule_result
...
'''
    ...
    # Compute via training reference (v06d15 reference feature extractor)
    _synth_ref = np.array(
        [_v06d15_option_features(_synth_obs, _synth_opts[oi], oi, _synth_n, _card_table) for oi in range(_synth_n)],
        dtype=np.float32
    )
...
_v06d15_card_meta_lookup = {{}}
try:
    from cg.api import all_card_data as _v06d15_acd
    for _c in _v06d15_acd():
...
def _v06d15_features_batch(obs, opts):
    n = len(opts)
    if n == 0: return _v06d15_np.zeros((0,96), dtype=_v06d15_np.float32)
    X = _v06d15_np.zeros((n,96), dtype=_v06d15_np.float32)
    cur = obs.get('current') or {{}}; sel = obs.get('select') or {{}}
    your = int(cur.get('yourIndex',0) or 0); opp = 1-your
    mine = _v06d15_player_summary(obs,your); theirs = _v06d15_player_summary(obs,opp)
    context = int(sel.get('context',0) or 0); denom = float(max(1,n-1))
    X[:,0]=1.0; X[:,1]=min(1.0,n/32.0)
    X[:,2]=_v06d15_np.minimum(1.0,_v06d15_np.arange(n,dtype=_v06d15_np.float32)/denom)
    X[:,3]=_v06d15_to_float(sel.get('minCount'),0.0)/8.0; X[:,4]=_v06d15_to_float(sel.get('maxCount'),0.0)/8.0
    X[:,5]=min(1.0,_v06d15_to_float(cur.get('turn'),0.0)/20.0)
    X[:,6]=min(1.0,_v06d15_to_float(cur.get('turnActionCount'),0.0)/32.0)
    X[:,7]=min(1.0,_v06d15_to_float(obs.get('step'),0.0)/512.0)
    ...
    opt_types=_v06d15_np.array([int((o or {{}}).get('type',0) or 0) for o in opts],dtype=_v06d15_np.int32)
    areas=_v06d15_np.array([int((o or {{}}).get('area',0) or 0) for o in opts],dtype=_v06d15_np.int32)
    card_ids=_v06d15_np.array([_v06d15_card_id_from_area(obs,o if isinstance(o,dict) else {{}}) for o in opts],dtype=_v06d15_np.int32)
    X[:,26]=(opt_types%32)/32.0; X[:,27]=(areas%16)/16.0
    X[:,28]=_v06d15_np.array([_v06d15_to_float((o or {{}}).get('index'),0.0) for o in opts],dtype=_v06d15_np.float32)/32.0
    X[:,29]=_v06d15_np.array([_v06d15_to_float((o or {{}}).get('inPlayIndex'),0.0) for o in opts],dtype=_v06d15_np.float32)/16.0
    X[:,30]=_v06d15_np.array([_v06d15_to_float((o or {{}}).get('number'),0.0) for o in opts],dtype=_v06d15_np.float32)/16.0
    X[:,31]=_v06d15_np.array([_v06d15_to_float((o or {{}}).get('attackId'),0.0) for o in opts],dtype=_v06d15_np.float32)/4096.0
    X[:,32]=_v06d15_np.array([_v06d15_to_float((o or {{}}).get('abilityId'),0.0) for o in opts],dtype=_v06d15_np.float32)/4096.0
    row_idx=_v06d15_np.arange(n)
    X[row_idx,33+(opt_types%16)]=1.0
    X[row_idx,49+(areas%12)]=1.0
    for _oi in range(n):
        _cid=int(card_ids[_oi])
        X[_oi,61:96]=_v06d15_card_meta_lookup.get(_cid,_V06D15_ZERO_META)
    return X

obs = json.loads(_v06d15_sys.argv[1])
opts = obs['select']['option']
X = _v06d15_features_batch(obs, opts)
print(json.dumps(X.tolist()))
'''

    import subprocess as _v06d15b_subp
    _cc_script_path = OUTPUT_DIR / '_v06d15b_cc_test.py'
    _cc_script_path.write_text(_crosscheck_script, encoding='utf-8')
    import sys as _sys_cc
    _cc_result = _v06d15b_subp.run(
    ...
    with _v06d15b_tarfile.open(_rule_only_archive) as _t:
    ...
    if '_v06d15_rule_agent_fn' in _base_src:
        raise RuntimeError('base source already contains v06d15 injection')
    ...
    def _build_guarded_torch_archive(main_source, deck, model_pt_path, archive_path):
        ...
        _tmp_main = WORKING_DIR / '_v06d15b_tmp_main.py'
        _tmp_deck = WORKING_DIR / '_v06d15b_tmp_deck.csv'
        ...
        with _v06d15b_tarfile.open(_ap, 'w:gz') as _tar:
        ...
        with _v06d15b_tarfile.open(_ap, 'r:gz') as _tar:
    ...
    ARTIFACT_PATHS['v06d15_guarded_submission_archive'] = str(_guarded_archive_path)

    # ------------------------------------------------------------------
    # Kaggle raw Python loader check
    # ------------------------------------------------------------------
    import tempfile as _v06d15b_tempfile
    from kaggle_environments.agent import get_last_callable as _v06d15_get_last_callable
    ...
    with _v06d15b_tempfile.TemporaryDirectory(prefix='v06d15_loader_') as _td:
        _extract_dir = Path(_td)
        with _v06d15b_tarfile.open(_guarded_archive_path, 'r:gz') as _tar:
        ...
        _selected_callable = _v06d15_get_last_callable(_archived_raw, path=str(_archived_main_path))
        ...
        _loader_model_loaded = bool(_selected_globals.get('_V06D15_MODEL_LOADED', False))
        _loader_model_error = str(_selected_globals.get('_v06d15_load_err_msg', ''))
        _loader_model_path = str(_selected_globals.get('_v06d15_model_path', ''))
    ...
    ARTIFACT_PATHS['v06d15_loader_validation_report'] = write_json(
        OUTPUT_DIR / 'kaggle_loader_validation_report.json', _loader_validation_report)

    # ------------------------------------------------------------------
    # Local smoke eval
    # ------------------------------------------------------------------
    _n_smoke_games = int(os.environ.get('V06D15_SMOKE_GAMES', '20'))
    _guarded_mod = import_agent_from_source(_guarded_src, 'v06d15_guarded')
    _torch_load_ok = bool(getattr(_guarded_mod, '_V06D15_MODEL_LOADED', False))
    if not _torch_load_ok:
        _load_err = getattr(_guarded_mod, '_v06d15_load_err_msg', 'unknown')
        raise RuntimeError(f'guarded agent: model failed to load: {_load_err}')
    print(f'guarded agent loaded: _V06D15_MODEL_LOADED={_torch_load_ok}')
    ...
    _latencies_ms = list(getattr(_guarded_mod, '_v06d15_latencies_ms', []))
    _action_changes = int(getattr(_guarded_mod, '_v06d15_action_changes', 0))
    _shadow_disagreements = int(getattr(_guarded_mod, '_v06d15_shadow_disagreements', 0))
    _shadow_type_counts = dict(getattr(_guarded_mod, '_v06d15_shadow_type_counts', {}))
    _decisions_seen = int(getattr(_guarded_mod, '_v06d15_decisions_seen', 0))
    ...
    ARTIFACT_PATHS['v06d15_runtime_probe_report'] = write_json(
    ...
    ARTIFACT_PATHS['v06d15_main_hybrid_report'] = write_json(
    ...
    ARTIFACT_PATHS['v06d15_promotion_decision'] = write_json(
...
except Exception as _v06d15b_exc:
    MAIN_HYBRID_REPORT['runtime_probe'] = {'status': 'error', 'error': repr(_v06d15b_exc),
                                            'traceback': traceback.format_exc(limit=8)}
    ARTIFACT_PATHS['v06d15_runtime_probe_report'] = write_json(
        OUTPUT_DIR / 'main_runtime_probe_report.json', MAIN_HYBRID_REPORT.get('runtime_probe', {}))
    print('v0-06d15 Part B FAILED:', repr(_v06d15b_exc))
    raise
