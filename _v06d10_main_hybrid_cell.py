# =====================================================================
# v0-06d10 Part A: MAIN replay-supervised guarded hybrid
# Early stopping on valid top-1 (patience=5, max_epochs=100).
# Best checkpoint restored at end of training.
# Infrastructure: same NumPy vectorized extraction + GPU pre-loading as v0-06d9.
# =====================================================================
...
def _v06d7_load_agent_from_archive(archive_path):
    ...
    spec = _v06d7_ilu.spec_from_file_location('v06d10_main_learning_rule_agent', str(mod_path))
...
try:
    ...
    _max_decisions = int(os.environ.get('V06D10_MAIN_MAX_DECISIONS', os.environ.get('V06D9_MAIN_MAX_DECISIONS', '120000')))
    _max_options = int(os.environ.get('V06D10_MAIN_MAX_OPTIONS', os.environ.get('V06D9_MAIN_MAX_OPTIONS', '1600000')))
    _rng = random.Random(20260624)

    # ------------------------------------------------------------------
    # Feature extraction (vectorized, same as v0-06d9)
    # ------------------------------------------------------------------
    _t0_extract = _v06d10_time.perf_counter()
    ...
    _t1_extract = _v06d10_time.perf_counter()
    _extract_time_sec = _t1_extract - _t0_extract
    print(f'v0-06d10 feature extraction: {_extract_time_sec:.1f}s, {_cursor} option rows, {_decision_id} decisions')
    ...
    if _check_mismatches > 0:
        raise RuntimeError(f'feature cross-check FAILED: {_check_mismatches} mismatches')
    ...
    ARTIFACT_PATHS['v06d10_main_feature_matrix'] = str(_feat_npy_path)
    ARTIFACT_PATHS['v06d10_main_labels'] = str(_label_npy_path)

    _feature_spec = {
        'feature_dim': _feature_dim,
        'target': 'recorded replay MAIN option index for alakazam actor',
        'extraction_method': 'numpy_preallocated_vectorized_v06d10',
        'crosscheck_passed': True,
        'crosscheck_max_abs_error': float(_check_max_err),
    }
    ARTIFACT_PATHS['v06d10_main_feature_spec'] = write_json(OUTPUT_DIR / 'main_feature_spec.json', _feature_spec)
    ...
    ARTIFACT_PATHS['v06d10_main_dataset_report'] = write_json(OUTPUT_DIR / 'main_dataset_report.json', _dataset_report)
    ARTIFACT_PATHS['v06d10_main_decision_rows'] = safe_save_table(_decision_df, OUTPUT_DIR / 'main_decision_rows.parquet')
    ...
    _max_epochs = int(os.environ.get('V06D10_MAIN_EPOCHS', '100'))
    _patience = int(os.environ.get('V06D10_MAIN_PATIENCE', '5'))
    _batch = int(os.environ.get('V06D10_MAIN_BATCH_SIZE', '8192'))
    _lr = float(os.environ.get('V06D10_MAIN_LR', '0.002'))

    # GPU pre-loading
    _t0_gpu_load = _v06d10_time.perf_counter()
    try:
        ...
        print(f'GPU pre-load ok: train={tuple(_X_train_gpu.shape)} valid={tuple(_X_valid_gpu.shape)} device={_device}')
    except RuntimeError as _gpu_err:
        print(f'GPU pre-load failed ({_gpu_err}), CPU fallback')
    ...
    _t1_gpu_load = _v06d10_time.perf_counter()
    _gpu_load_time_sec = _t1_gpu_load - _t0_gpu_load

    # Pre-compute valid decision boundaries in valid-only space for per-epoch top-1
    _valid_decisions_df = _decision_df[_decision_df['split'].astype(str) == 'valid'].copy()
    _full_valid_indices = np.where(_valid_mask)[0]
    _orig_to_valid_pos = {int(idx): pos for pos, idx in enumerate(_full_valid_indices)}
    _vd_local_starts = np.array(
        [_orig_to_valid_pos[int(r['option_start'])] for _, r in _valid_decisions_df.iterrows()],
        dtype=np.int64,
    )
    _vd_local_ends = _vd_local_starts + _valid_decisions_df['n_options'].values.astype(np.int64)
    _vd_chosen = _valid_decisions_df['chosen_index'].values.astype(np.int32)
    _n_valid_decisions = len(_valid_decisions_df)
    ...
    # ------------------------------------------------------------------
    # Training loop with early stopping on valid top-1
    # ------------------------------------------------------------------
    _t0_train = _v06d10_time.perf_counter()
    _history = []
    ...
    for _ep in range(_max_epochs):
        ...
        with torch.no_grad():
            ...
            _valid_logits_ep = np.empty(_n_valid, dtype=np.float32)
            for _st in range(0, _n_valid, _batch):
                _xb = _X_valid_gpu[_st:_st + _batch]
                if _X_valid_gpu.device.type != 'cuda':
                    _xb = _xb.to(_device)
                _valid_logits_ep[_st:_st + _batch] = _model(_xb).detach().cpu().numpy()

        _ep_hits = 0
        for _vi in range(_n_valid_decisions):
            _vs, _ve = int(_vd_local_starts[_vi]), int(_vd_local_ends[_vi])
            if np.argmax(_valid_logits_ep[_vs:_ve]) == _vd_chosen[_vi]:
                _ep_hits += 1
        _ep_valid_top1 = _ep_hits / max(1, _n_valid_decisions)
        _ep_valid_loss = float(np.mean(_vl))
        _ep_train_loss = float(np.mean(_losses))
        _epochs_run = _ep + 1
        ...
        print(f'  ep {_ep:3d}: train={_ep_train_loss:.4f} valid_loss={_ep_valid_loss:.4f} valid_top1={_ep_valid_top1:.4f}'
              + (' *' if _ep_valid_top1 > _best_valid_top1 else f'  (patience {_patience_counter+1}/{_patience})'))

        if _ep_valid_top1 > _best_valid_top1:
    ...
    if _best_state_dict is not None:
        ...
        print(f'restored best checkpoint from epoch {_best_epoch} (valid_top1={_best_valid_top1:.4f})')
    _model.eval()

    _t1_train = _v06d10_time.perf_counter()
    _train_time_sec = _t1_train - _t0_train

    # ------------------------------------------------------------------
    # Inference on full dataset
    # ------------------------------------------------------------------
    _t0_infer = _v06d10_time.perf_counter()
    ...
    _t1_infer = _v06d10_time.perf_counter()
    ...
    print(f'timing: extract={_extract_time_sec:.1f}s gpu={_gpu_load_time_sec:.1f}s train={_train_time_sec:.1f}s({_epochs_run}ep) infer={_infer_time_sec:.1f}s')
    print(f'  best_epoch={_best_epoch} best_valid_top1={_best_valid_top1:.4f}')

    _model_path = OUTPUT_DIR / (RUN_PREFIX + '-main_option_scorer.pt')
    torch.save(_model.state_dict(), _model_path)
    ARTIFACT_PATHS['v06d10_main_option_scorer'] = str(_model_path)

    # ------------------------------------------------------------------
    # Prediction table and per-bucket evaluation (same as v0-06d9)
    # ------------------------------------------------------------------
    _pred_rows = []
    for _idx, _r in _decision_df.iterrows():
    ...
    ARTIFACT_PATHS['v06d10_main_holdout_predictions'] = safe_save_table(
    ...
    def _metric_row(df, bucket_name, bucket_value):
        ...
        return {
            ...
            'rule_top1': float(df['rule_hit'].mean()), 'model_top1': float(df['model_top1_hit'].mean()),
            'model_top3': float(df['model_top3_hit'].mean()), 'model_chosen_rank': float(df['model_chosen_rank'].mean()),
    ...
    ARTIFACT_PATHS['v06d10_main_eval_by_bucket'] = safe_save_table(_eval_df, OUTPUT_DIR / 'main_eval_by_bucket.parquet')

    # ------------------------------------------------------------------
    # Hybrid evaluation (same guard logic as v0-06d9)
    # ------------------------------------------------------------------
    _safe_override_types = [x.strip().upper() for x in
        os.environ.get('V06D10_SAFE_MAIN_OVERRIDE_TYPES', 'PLAY,ATTACH,EVOLVE').split(',') if x.strip()]
    _rule_veto_types = [x.strip().upper() for x in
        os.environ.get('V06D10_RULE_VETO_MAIN_TYPES', 'ATTACK,END,ABILITY').split(',') if x.strip()]
    _prob_grid = [float(x) for x in
        os.environ.get('V06D10_HYBRID_PROB_GRID', '0.35,0.45,0.55,0.65,0.75,0.85').split(',') if str(x).strip()]
    _margin_grid = [float(x) for x in
        os.environ.get('V06D10_HYBRID_MARGIN_GRID', '0.00,0.15,0.30,0.50,0.75,1.00').split(',') if str(x).strip()]
    ...
    def _hybrid_eval(df, prob_threshold, margin_threshold, label):
        ...
        return {
            ...
            'hybrid_top1': _hybrid_t1, 'hybrid_minus_rule_top1': float(_hybrid_t1 - _rule_t1),
            ...
            'danger_rule_top1': _danger_rule, 'danger_hybrid_top1': _danger_hybrid,
            'danger_hybrid_minus_rule': None if _danger_rule is None or _danger_hybrid is None else float(_danger_hybrid - _danger_rule),
            'safe_chosen_rule_top1': _safe_rule, 'safe_chosen_hybrid_top1': _safe_hybrid,
    ...
    ARTIFACT_PATHS['v06d10_main_hybrid_threshold_grid'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d10_main_hybrid_eval_by_bucket'] = safe_save_table(
    ...
    _beats_rule_buckets = _eval_df[
        (_eval_df['bucket_name'] == 'chosen_option_type_name') &
        (_eval_df['n'] >= 100) & (_eval_df['model_minus_rule_top1'] > 0.01)
    ].to_dict('records')

    _holdout_model_top1 = float(_hold_summary.get('model_top1', 0.0) or 0.0)
    _v06d9_baseline = 0.5012
    _quality_ok = _holdout_model_top1 >= _v06d9_baseline - 0.01

    MAIN_HYBRID_REPORT = {
        'status': 'ok',
        'version': RUN_PREFIX,
        'purpose': 'early stopping on valid top-1 + gpu pre-loading; same features/architecture as v0-06d9',
        ...
        'v06d9_baseline_holdout_model_top1': _v06d9_baseline,
        'holdout_model_top1_vs_v06d9': float(_holdout_model_top1 - _v06d9_baseline),
        'quality_ok': bool(_quality_ok),
        'runtime_adoption': 'pending_part_b_guarded_runtime_probe',
        'fresh_training': True,
        'reuses_v0_06d9_weights_or_predictions': False,
    }
    MAIN_LEARNING_REPORT = MAIN_HYBRID_REPORT
    ARTIFACT_PATHS['v06d10_main_model_report'] = write_json(OUTPUT_DIR / 'main_model_report.json', MAIN_HYBRID_REPORT)
    ARTIFACT_PATHS['v06d10_main_hybrid_report'] = write_json(OUTPUT_DIR / 'main_hybrid_report.json', MAIN_HYBRID_REPORT)

    print('v0-06d10 Part A status: ok')
    print(f'  epochs_run={_epochs_run} best_epoch={_best_epoch} best_valid_top1={_best_valid_top1:.4f}')
    print(f'  holdout model_top1={_holdout_model_top1:.4f} (v06d9={_v06d9_baseline}, quality_ok={_quality_ok})')
...
except Exception as _v06d10_exc:
    MAIN_HYBRID_REPORT = {'status': 'error', 'error': repr(_v06d10_exc), 'traceback': traceback.format_exc(limit=8)}
    MAIN_LEARNING_REPORT = MAIN_HYBRID_REPORT
    ARTIFACT_PATHS['v06d10_main_model_report'] = write_json(OUTPUT_DIR / 'main_model_report.json', MAIN_HYBRID_REPORT)
    print('v0-06d10 Part A FAILED:', repr(_v06d10_exc))
    raise
