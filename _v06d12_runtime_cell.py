# =====================================================================
# v0-06d12 Part B: Guarded runtime probe + Kaggle loader entrypoint check
...
# Feature extractor updated to v06d12 layout (card metadata, no card_id hash).
# Guard scope: ATTACK/END/RETREAT enabled (ABILITY hard-vetoed).
# =====================================================================
import shutil as _v06d12b_shutil
import tarfile as _v06d12b_tarfile
...
try:
    ...
    _v06d12b_shutil.copy2(str(_model_pt_src), str(_local_model_dst))
    ...
    _GUARD_INJECTION = f'''
# ============================================================
# v0-06d12 Guarded MAIN option scorer — appended to rule agent
# Changes vs v06d11:
#   - Feature layout v06d12: card metadata [61-95], no card_id hash
#   - Guard scope: ATTACK/END/RETREAT enabled (ABILITY hard-vetoed)
# ============================================================
_v06d12_rule_agent_fn = agent  # save original

_V06D12_MODEL_LOADED = False
_v06d12_model = None

try:
    import numpy as _v06d12_np
    import time as _v06d12_time
    import torch as _v06d12_torch
    import torch.nn as _v06d12_nn
    import sys as _v06d12_sys
    from pathlib import Path as _v06d12_Path

    class _V06D12MainScorer(_v06d12_nn.Module):
        def __init__(self):
            super().__init__()
            self.net = _v06d12_nn.Sequential(
                _v06d12_nn.Linear(96, 512), _v06d12_nn.ReLU(),
                _v06d12_nn.Linear(512, 384), _v06d12_nn.ReLU(),
                _v06d12_nn.Linear(384, 256), _v06d12_nn.ReLU(),
                _v06d12_nn.Linear(256, 1),
            )
        def forward(self, x):
            return self.net(x).squeeze(-1)

    _v06d12_device = _v06d12_torch.device(\'cpu\')
    _v06d12_base_dir = _v06d12_Path(globals().get(\'__file__\', _v06d12_sys.path[-1])).parent
    if not (_v06d12_base_dir / \'models\' / \'main_option_scorer.pt\').exists():
        _v06d12_base_dir = _v06d12_Path(_v06d12_sys.path[-1])
    _v06d12_model_path = _v06d12_base_dir / \'models\' / \'main_option_scorer.pt\'
    _v06d12_model = _V06D12MainScorer().to(_v06d12_device)
    _v06d12_model.load_state_dict(
        _v06d12_torch.load(str(_v06d12_model_path), map_location=\'cpu\', weights_only=True)
    )
    _v06d12_model.eval()

    # Build card metadata lookup from bundled cg package
    _V06D12_CARD_META_DIM = 35
    _V06D12_CARD_META_OFFSET = 61
    _v06d12_card_meta_lookup = {{}}
    try:
        if str(_v06d12_base_dir) not in _v06d12_sys.path:
            _v06d12_sys.path.insert(0, str(_v06d12_base_dir))
        from cg.api import all_card_data as _v06d12_all_card_data
        for _c in _v06d12_all_card_data():
            _cid = int(_c.cardId)
            _vec = [0.0] * _V06D12_CARD_META_DIM
    ...
    _V06D12_MODEL_LOADED = True
except Exception as _v06d12_load_err:
    _V06D12_MODEL_LOADED = False
    _v06d12_load_err_msg = repr(_v06d12_load_err)
...
def _v06d12_features_batch(obs, opts):
    """Extract (n_opts, 96) feature matrix — v06d12 layout (card metadata, no card_id hash)."""
    n = len(opts)
    if n == 0:
        return _v06d12_np.zeros((0, 96), dtype=_v06d12_np.float32)
    ...
    X[:, 3] = _v06d12_to_float(sel.get(\'minCount\'), 0.0) / 8.0
    X[:, 4] = _v06d12_to_float(sel.get(\'maxCount\'), 0.0) / 8.0
    ...
    card_ids = _v06d12_np.array([
        _v06d12_card_id_from_area(obs, o if isinstance(o, dict) else {{}}) for o in opts
    ], dtype=_v06d12_np.int32)
    opt_indices = _v06d12_np.array([_v06d12_to_float((o or {{}}).get(\'index\'), 0.0) for o in opts], dtype=_v06d12_np.float32)
    in_plays = _v06d12_np.array([_v06d12_to_float((o or {{}}).get(\'inPlayIndex\'), 0.0) for o in opts], dtype=_v06d12_np.float32)
    numbers = _v06d12_np.array([_v06d12_to_float((o or {{}}).get(\'number\'), 0.0) for o in opts], dtype=_v06d12_np.float32)
    attack_ids = _v06d12_np.array([_v06d12_to_float((o or {{}}).get(\'attackId\'), 0.0) for o in opts], dtype=_v06d12_np.float32)
    ability_ids = _v06d12_np.array([_v06d12_to_float((o or {{}}).get(\'abilityId\'), 0.0) for o in opts], dtype=_v06d12_np.float32)
    # [26-32]: option numerics (v06d12 layout — no card_id continuous)
    ...
    for _oi in range(n):
        ...
        _meta = _v06d12_card_meta_lookup.get(_cid, _V06D12_ZERO_META)
        X[_oi, 61:96] = _meta
    return X


_V06D12_SAFE_OVERRIDE_TYPES = frozenset({{7, 8, 9, 12}})  # PLAY, ATTACH, EVOLVE, RETREAT
_V06D12_HARD_VETO_TYPES = frozenset({{10}})  # ABILITY only
_V06D12_PROB_THRESHOLD = {_sel_prob_b}
_V06D12_MARGIN_THRESHOLD = {_sel_margin_b}

_v06d12_action_changes = 0
_v06d12_decisions_seen = 0
_v06d12_latencies_ms = []


def agent(obs_dict: dict) -> list:
    global _v06d12_action_changes, _v06d12_decisions_seen, _v06d12_latencies_ms
    rule_result = _v06d12_rule_agent_fn(obs_dict)
    if not _V06D12_MODEL_LOADED:
        return rule_result
    try:
        ...
        if rule_type in _V06D12_HARD_VETO_TYPES:
            return rule_result
        t0 = _v06d12_time.perf_counter()
        X = _v06d12_features_batch(obs_dict, opts)
        with _v06d12_torch.no_grad():
            logits = _v06d12_model(
                _v06d12_torch.tensor(X, dtype=_v06d12_torch.float32)
            ).numpy()
        _v06d12_latencies_ms.append((_v06d12_time.perf_counter() - t0) * 1000.0)
        _v06d12_decisions_seen += 1
        top1 = int(_v06d12_np.argmax(logits))
        if top1 == rule_idx:
            return rule_result
        top1_opt = opts[top1] if (0 <= top1 < n and isinstance(opts[top1], dict)) else {{}}
        top1_type = int(top1_opt.get(\'type\', -1))
        if top1_type not in _V06D12_SAFE_OVERRIDE_TYPES or top1_type in _V06D12_HARD_VETO_TYPES:
            return rule_result
        sorted_l = _v06d12_np.sort(logits)[::-1]
        prob = float(1.0 / (1.0 + _v06d12_np.exp(-sorted_l[0])))
        margin = float(sorted_l[0] - sorted_l[1]) if len(sorted_l) > 1 else 0.0
        if prob < _V06D12_PROB_THRESHOLD or margin < _V06D12_MARGIN_THRESHOLD:
            return rule_result
        _v06d12_action_changes += 1
        return [top1]
    except Exception:
        return rule_result
...
'''
    ...
    # Compute via training reference (v06d12 reference feature extractor)
    _synth_ref = np.array(
        [_v06d12_option_features(_synth_obs, _synth_opts[oi], oi, _synth_n, _card_table) for oi in range(_synth_n)],
        dtype=np.float32
    )
...
'''
    ...
    import subprocess as _v06d12b_subp
    _cc_script_path = OUTPUT_DIR / '_v06d12b_cc_test.py'
    _cc_script_path.write_text(_crosscheck_script, encoding='utf-8')
    import sys as _sys_cc
    _cc_result = _v06d12b_subp.run(
    ...
    with _v06d12b_tarfile.open(_rule_only_archive) as _t:
    ...
    if '_v06d12_rule_agent_fn' in _base_src:
        raise RuntimeError('base source already contains v06d12 injection')
    ...
    def _build_guarded_torch_archive(main_source, deck, model_pt_path, archive_path):
        ...
        _tmp_main = WORKING_DIR / '_v06d12b_tmp_main.py'
        _tmp_deck = WORKING_DIR / '_v06d12b_tmp_deck.csv'
        ...
        with _v06d12b_tarfile.open(_ap, 'w:gz') as _tar:
        ...
        with _v06d12b_tarfile.open(_ap, 'r:gz') as _tar:
    ...
    ARTIFACT_PATHS['v06d12_guarded_submission_archive'] = str(_guarded_archive_path)

    # ------------------------------------------------------------------
    # Kaggle raw Python loader check
    # ------------------------------------------------------------------
    import tempfile as _v06d12b_tempfile
    from kaggle_environments.agent import get_last_callable as _v06d12_get_last_callable
    ...
    with _v06d12b_tempfile.TemporaryDirectory(prefix='v06d12_loader_') as _td:
        _extract_dir = Path(_td)
        with _v06d12b_tarfile.open(_guarded_archive_path, 'r:gz') as _tar:
        ...
        _selected_callable = _v06d12_get_last_callable(_archived_raw, path=str(_archived_main_path))
        ...
        _loader_model_loaded = bool(_selected_globals.get('_V06D12_MODEL_LOADED', False))
        _loader_model_error = str(_selected_globals.get('_v06d12_load_err_msg', ''))
        _loader_model_path = str(_selected_globals.get('_v06d12_model_path', ''))
    ...
    ARTIFACT_PATHS['v06d12_loader_validation_report'] = write_json(
        OUTPUT_DIR / 'kaggle_loader_validation_report.json', _loader_validation_report)

    # ------------------------------------------------------------------
    # Local smoke eval
    # ------------------------------------------------------------------
    _n_smoke_games = int(os.environ.get('V06D12_SMOKE_GAMES', '20'))
    _guarded_mod = import_agent_from_source(_guarded_src, 'v06d12_guarded')
    _torch_load_ok = bool(getattr(_guarded_mod, '_V06D12_MODEL_LOADED', False))
    if not _torch_load_ok:
        _load_err = getattr(_guarded_mod, '_v06d12_load_err_msg', 'unknown')
        raise RuntimeError(f'guarded agent: model failed to load: {_load_err}')
    print(f'guarded agent loaded: _V06D12_MODEL_LOADED={_torch_load_ok}')
    ...
    _latencies_ms = list(getattr(_guarded_mod, '_v06d12_latencies_ms', []))
    _action_changes = int(getattr(_guarded_mod, '_v06d12_action_changes', 0))
    _decisions_seen = int(getattr(_guarded_mod, '_v06d12_decisions_seen', 0))
    ...
    ARTIFACT_PATHS['v06d12_runtime_probe_report'] = write_json(
    ...
    MAIN_HYBRID_REPORT['all_gates_ok'] = bool(
        MAIN_HYBRID_REPORT.get('quality_ok', False) and _smoke_gate_ok and _loader_entrypoint_ok)
    ARTIFACT_PATHS['v06d12_main_hybrid_report'] = write_json(
    ...
    if _all_gates and _danger_gate_ok:
        if _holdout_hybrid_top1 >= _v06d11_hybrid_baseline - 0.01 and _holdout_model_top1 >= 0.45:
    ...
    ARTIFACT_PATHS['v06d12_promotion_decision'] = write_json(
    ...
    print(f'  holdout_hybrid_top1={_holdout_hybrid_top1:.4f} (v06d11={_v06d11_hybrid_baseline})')
    print(f'  attack_delta={_attack_delta}  danger_gate_ok={_danger_gate_ok}  all_gates_ok={_all_gates}')

except Exception as _v06d12b_exc:
    MAIN_HYBRID_REPORT['runtime_probe'] = {'status': 'error', 'error': repr(_v06d12b_exc),
                                            'traceback': traceback.format_exc(limit=8)}
    ARTIFACT_PATHS['v06d12_runtime_probe_report'] = write_json(
        OUTPUT_DIR / 'main_runtime_probe_report.json', MAIN_HYBRID_REPORT.get('runtime_probe', {}))
    print('v0-06d12 Part B FAILED:', repr(_v06d12b_exc))
    raise
