"""Build pokemon-20260623-v0-06d8.ipynb from v0-06d6 + fresh MAIN hybrid cell."""
import json


SRC = "/kaggle/working/pokemon-20260623-v0-06d6.ipynb"
DST = "/kaggle/working/pokemon-20260623-v0-06d8.ipynb"
CELL = "/kaggle/working/_v06d8_main_hybrid_cell.py"

nb = json.load(open(SRC))
main_src = open(CELL).read()

nb["cells"][0]["source"] = [
    "# v0-06d8 Fresh MAIN Guarded Hybrid Notebook\n",
    "\n",
    "This notebook continues from the v0-06d6 replay-agreement diagnostic line. It trains a fresh PyTorch option scorer on real replay `MAIN` decisions, selects guarded hybrid thresholds on valid, and evaluates the hybrid on holdout. The v0-06d7 model weights or predictions are not reused.\n",
]
...
for cell in nb["cells"]:
    ...
    src = src.replace(
        ...
        "spec = _ilu.spec_from_file_location('v06d8_replay_agreement_agent', str(mod_path))",
    )
...
"""
if "v06d8_main_hybrid_report" not in final_src:
...
md = {
    ...
    "metadata": {},
    "source": [
        "## v0-06d8: Fresh MAIN Replay-Supervised Guarded Hybrid\n",
        "\n",
        "Builds a real replay supervised `MAIN` dataset, trains a fresh compact PyTorch option scorer, selects guard thresholds on valid, and evaluates a rule-default hybrid on holdout. This version does not enable torch in the submission archive.\n",
    ],
}
code = {
    "cell_type": "code",
