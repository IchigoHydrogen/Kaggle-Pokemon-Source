"""Build pokemon-20260624-v0-06d13.ipynb from v0-06d12."""
import json
import os

SRC = "/home/ei/wkdir/2422005/kaggle/working/pokemon-20260624-v0-06d12/pokemon-20260624-v0-06d12.ipynb"
DST = "/home/ei/wkdir/2422005/kaggle/working/pokemon-20260624-v0-06d13/pokemon-20260624-v0-06d13.ipynb"
CELL_A = "/home/ei/wkdir/2422005/kaggle/working/_v06d13_main_cell.py"
CELL_B = "/home/ei/wkdir/2422005/kaggle/working/_v06d13_runtime_cell.py"
...
nb["cells"][0]["source"] = [
    "# v0-06d13 Rule-family conditioned ATTACK/END diagnostics\n",
    "\n",
    "Learning-focused diagnostic version. Keeps v0-06d12 per-decision CE loss, "
    "card metadata features, model architecture, deck, and deterministic split. "
    "Adds rule-option-type conditioned evaluation for ATTACK within-type reranking, "
    "END deferral, and cross-family contamination. Runtime is shadow_torch_policy; "
    "model output is measured but does not change submitted actions.\n",
]

config = "".join(nb["cells"][1]["source"])
config = config.replace(
    "EXPERIMENT_NAME = 'v0_06d12_ce_card_meta_extended_guard'",
    "EXPERIMENT_NAME = 'v0_06d13_rule_family_attack_end_diagnostics'",
)
config = config.replace(
    "RUN_PREFIX = os.environ.get('V05_RUN_PREFIX', 'pokemon-20260624-v0-06d12')",
    "RUN_PREFIX = os.environ.get('V05_RUN_PREFIX', 'pokemon-20260624-v0-06d13')",
)
nb["cells"][1]["source"] = [line + "\n" for line in config.splitlines()]

config_joined = "".join(nb["cells"][1]["source"])
assert "pokemon-20260624-v0-06d13" in config_joined
assert "v0_06d13_rule_family_attack_end_diagnostics" in config_joined
...
nb["cells"][section_header_idx]["source"] = [
    ...
    "formulation, then adds rule-option-type conditioned reports for ATTACK within-type "
    "reranking, END deferral, and cross-family contamination. Thresholds are selected "
    "on valid only and reported on holdout.\n",
    "\n",
    "**Part B** — Runtime: Builds a shadow_torch_policy archive, validates Kaggle loader "
    "entrypoint, and runs local smoke eval with latency and shadow-disagreement measurement. "
    "Model output does not change actions in this version.\n",
]
...
for i, cell in enumerate(nb["cells"]):
    src = "".join(cell.get("source", []))
    if cell.get("cell_type") == "markdown" and "v0-06d13: Rule-family conditioned" in src:
        insert_before = i + 1
        break
if insert_before is None:
    raise RuntimeError("could not locate v06d13 insertion point")
...
part_b_markdown = {
    ...
    "metadata": {},
    "source": [
        "## v0-06d13 Part B: Shadow Runtime Probe + Kaggle Loader Check\n",
        "\n",
        ...
        "plus shadow disagreement counts.\n",
    ],
}
part_b_code = {
    "cell_type": "code",
...
final_src = final_src.replace("v06d12_main_hybrid_report", "v06d13_main_hybrid_report")
final_src = final_src.replace("v06d12_main_hybrid_status", "v06d13_main_hybrid_status")
final_src = final_src.replace("v06d12_runtime_probe", "v06d13_runtime_probe")
final_src = final_src.replace("v06d12_promotion", "v06d13_promotion")
