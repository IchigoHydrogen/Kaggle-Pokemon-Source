...
import numpy as _v06d11_np
import torch as _v06d11_torch
import torch.nn as _v06d11_nn
import time as _v06d11_time
from pathlib import Path as _v06d11_Path

def _v06d11_to_float(x, default=0.0):
    try:
        if x is None: return float(default)
        return float(x)
    except Exception: return float(default)

def _v06d11_card_id_from_area(obs, option):
    try:
...
def _v06d11_player_summary(obs, player_idx):
...
def _v06d11_features_batch(obs, opts):
    n = len(opts)
    if n == 0: return _v06d11_np.zeros((0, 96), dtype=_v06d11_np.float32)
    X = _v06d11_np.zeros((n, 96), dtype=_v06d11_np.float32)
    cur = obs.get('current') or {}; sel = obs.get('select') or {}
    your = int(cur.get('yourIndex', 0) or 0); opp = 1 - your
    mine = _v06d11_player_summary(obs, your); theirs = _v06d11_player_summary(obs, opp)
    context = int(sel.get('context', 0) or 0); denom = float(max(1, n - 1))
    X[:, 0]=1.0; X[:, 1]=min(1.0, n/32.0)
    X[:, 2]=_v06d11_np.minimum(1.0, _v06d11_np.arange(n, dtype=_v06d11_np.float32)/denom)
    X[:, 3]=_v06d11_to_float(sel.get('minCount'),0.0)/8.0; X[:, 4]=_v06d11_to_float(sel.get('maxCount'),0.0)/8.0
    X[:, 5]=min(1.0, _v06d11_to_float(cur.get('turn'),0.0)/20.0)
    X[:, 6]=min(1.0, _v06d11_to_float(cur.get('turnActionCount'),0.0)/32.0)
    X[:, 7]=min(1.0, _v06d11_to_float(obs.get('step'),0.0)/512.0)
    ...
    opt_types=_v06d11_np.array([int((o or {}).get('type',0) or 0) for o in opts], dtype=_v06d11_np.int32)
    areas=_v06d11_np.array([int((o or {}).get('area',0) or 0) for o in opts], dtype=_v06d11_np.int32)
    card_ids=_v06d11_np.array([_v06d11_card_id_from_area(obs, o if isinstance(o, dict) else {}) for o in opts], dtype=_v06d11_np.int32)
    opt_indices=_v06d11_np.array([_v06d11_to_float((o or {}).get('index'),0.0) for o in opts], dtype=_v06d11_np.float32)
    in_plays=_v06d11_np.array([_v06d11_to_float((o or {}).get('inPlayIndex'),0.0) for o in opts], dtype=_v06d11_np.float32)
    numbers=_v06d11_np.array([_v06d11_to_float((o or {}).get('number'),0.0) for o in opts], dtype=_v06d11_np.float32)
    attack_ids=_v06d11_np.array([_v06d11_to_float((o or {}).get('attackId'),0.0) for o in opts], dtype=_v06d11_np.float32)
    ability_ids=_v06d11_np.array([_v06d11_to_float((o or {}).get('abilityId'),0.0) for o in opts], dtype=_v06d11_np.float32)
    ...
    row_idx=_v06d11_np.arange(n)
    X[row_idx, 34+(opt_types % 16)]=1.0; X[row_idx, 50+(areas % 12)]=1.0
    X[row_idx, 62+(card_ids % 32)]=1.0
    X[:, 94]=(opt_types == 13).astype(_v06d11_np.float32)
    X[:, 95]=(opt_types == 14).astype(_v06d11_np.float32)
    return X

import json, sys
obs = json.loads(sys.argv[1])
opts = obs['select']['option']
X = _v06d11_features_batch(obs, opts)
