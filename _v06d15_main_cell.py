# =====================================================================
# v0-06d15 Part A: Rule-family conditioned ATTACK/END diagnostics
#
...
#   3. Diagnostics: rule-option-type conditioned reports for ATTACK within-type
#      reranking, END deferral, and cross-family contamination.
# =====================================================================
import importlib.util as _v06d15_ilu
import tarfile as _v06d15_tarfile
import time as _v06d15_time
import torch.nn.functional as _v06d15_F
from collections import defaultdict as _v06d15_dd
...
_V06D15_CARD_META_DIM = 35
_V06D15_CARD_META_OFFSET = 61  # start index in 96-dim vector
_V06D15_FEAT_DIM = 96

def _v06d15_name(mapping, value, prefix):
    try:
        return mapping.get(int(value), prefix + '_' + str(value))
    except Exception:
        return prefix + '_' + str(value)

def _v06d15_norm_name(s):
    return re.sub(r'[^a-z0-9]', '', str(s).lower())

def _v06d15_rank_bucket(rank):
    if rank is None:
        return 'unranked'
    if rank <= 10:
        return 'top10'
...
def _v06d15_archetype_of_deck(deck):
...
def _v06d15_load_agent_from_archive(archive_path):
    with _v06d15_tarfile.open(archive_path) as t:
    ...
    spec = _v06d15_ilu.spec_from_file_location('v06d15_main_learning_rule_agent', str(mod_path))
    m = _v06d15_ilu.module_from_spec(spec)
    spec.loader.exec_module(m)
    return m, deck

def _v06d15_to_float(x, default=0.0):
    try:
        if x is None:
...
def _v06d15_card_id_from_area(obs, option):
    try:
...
def _v06d15_player_summary(obs, player_idx):
    cur = obs.get('current') or {}
    players = cur.get('players') or []
...
def _v06d15_card_meta_vec(card_id, card_table):
    """Return 35-dim card metadata vector for card_id."""
    vec = [0.0] * _V06D15_CARD_META_DIM
...
def _v06d15_option_features(obs, option, option_index, option_count, card_table):
    """Reference Python-loop feature extractor (cross-check only). 96 dims."""
    cur = obs.get('current') or {}
    select = obs.get('select') or {}
    your = int(cur.get('yourIndex', 0) or 0)
    opp = 1 - your
    mine = _v06d15_player_summary(obs, your)
    theirs = _v06d15_player_summary(obs, opp)
    context = int(select.get('context', 0) or 0)
    opt_type = int(option.get('type', 0) or 0)
    area = int(option.get('area', 0) or 0)
    card_id = _v06d15_card_id_from_area(obs, option)
    row = [0.0] * _V06D15_FEAT_DIM
    # [0-12]: global context
    row[0] = 1.0
    row[1] = min(1.0, option_count / 32.0)
    row[2] = min(1.0, option_index / max(1, option_count - 1))
    row[3] = _v06d15_to_float(select.get('minCount'), 0.0) / 8.0
    row[4] = _v06d15_to_float(select.get('maxCount'), 0.0) / 8.0
    row[5] = min(1.0, _v06d15_to_float(cur.get('turn'), 0.0) / 20.0)
    row[6] = min(1.0, _v06d15_to_float(cur.get('turnActionCount'), 0.0) / 32.0)
    row[7] = min(1.0, _v06d15_to_float(obs.get('step'), 0.0) / 512.0)
    row[8] = float(your)
    ...
    row[27] = (area % 16) / 16.0
    row[28] = _v06d15_to_float(option.get('index'), 0.0) / 32.0
    row[29] = _v06d15_to_float(option.get('inPlayIndex'), 0.0) / 16.0
    row[30] = _v06d15_to_float(option.get('number'), 0.0) / 16.0
    row[31] = _v06d15_to_float(option.get('attackId'), 0.0) / 4096.0
    row[32] = _v06d15_to_float(option.get('abilityId'), 0.0) / 4096.0
    # [33-48]: opt_type one-hot (16 dims)
    row[33 + (opt_type % 16)] = 1.0
    # [49-60]: area one-hot (12 dims)
    row[49 + (area % 12)] = 1.0
    # [61-95]: card metadata (35 dims)
    meta = _v06d15_card_meta_vec(card_id, card_table)
    for mi, mv in enumerate(meta):
        row[_V06D15_CARD_META_OFFSET + mi] = mv
    return row
...
try:
    ...
    _rule_mod, _our_deck = _v06d15_load_agent_from_archive(_archive)
    _rule_agent = _rule_mod.agent
    _our_arch = _v06d15_archetype_of_deck(_our_deck)

    # Load card metadata
    _t0_card = _v06d15_time.perf_counter()
    from cg.api import all_card_data as _v06d15_all_card_data
    _card_table = {c.cardId: c for c in _v06d15_all_card_data()}
    print(f'card table loaded: {len(_card_table)} cards in {_v06d15_time.perf_counter()-_t0_card:.2f}s')
    ...
    def _rank_of(name):
        info = _rank_info.get(_v06d15_norm_name(name), {}) if isinstance(_rank_info, dict) else {}
        r = info.get('ranking') if isinstance(info, dict) else None
        return int(r) if r is not None else None

    _max_decisions = int(os.environ.get('V06D15_MAIN_MAX_DECISIONS', '120000'))
    _max_options = int(os.environ.get('V06D15_MAIN_MAX_OPTIONS', '1600000'))
    ...
    # ------------------------------------------------------------------
    _t0_extract = _v06d15_time.perf_counter()
    _alloc_size = _max_options + 1024
    _X = np.zeros((_alloc_size, _V06D15_FEAT_DIM), dtype=np.float32)
    _y = np.zeros(_alloc_size, dtype=np.float32)
    _cursor = 0
    _decision_rows = []
    _option_meta_rows = []
    _decision_id = 0
    _stats = _v06d15_dd(int)
    _cross_check_buf = []
    ...
    for _cid in _known_card_ids:
        _card_meta_lookup[_cid] = np.array(_v06d15_card_meta_vec(_cid, _card_table), dtype=np.float32)
    _zero_meta = np.zeros(_V06D15_CARD_META_DIM, dtype=np.float32)
    ...
    for _fp in EPISODE_FILES:
        ...
        _seat_arch = {k: _v06d15_archetype_of_deck(v) for k, v in _seat_deck.items()}
        ...
        for _s in _steps:
            ...
            for _seat, _a in enumerate(_s):
                ...
                _opp_idx = 1 - _your
                _mine = _v06d15_player_summary(_obs, _your)
                _theirs = _v06d15_player_summary(_obs, _opp_idx)
                _context = int(_sel2.get('context', 0) or 0)

                # Shared decision-level scalars
                _f_bias = 1.0
                _f_opt_count = min(1.0, _nopt / 32.0)
                _f_min_count = _v06d15_to_float(_sel2.get('minCount'), 0.0) / 8.0
                _f_max_count = _v06d15_to_float(_sel2.get('maxCount'), 0.0) / 8.0
                _f_turn = min(1.0, _v06d15_to_float(_cur.get('turn'), 0.0) / 20.0)
                _f_turn_act = min(1.0, _v06d15_to_float(_cur.get('turnActionCount'), 0.0) / 32.0)
                _f_step = min(1.0, _v06d15_to_float(_obs.get('step'), 0.0) / 512.0)
                _f_your = float(_your)
                ...
                _card_ids = np.array([_v06d15_card_id_from_area(_obs, _o if isinstance(_o, dict) else {}) for _o in _opts], dtype=np.int32)
                _opt_indices = np.array([_v06d15_to_float((_o or {}).get('index'), 0.0) for _o in _opts], dtype=np.float32)
                _in_plays = np.array([_v06d15_to_float((_o or {}).get('inPlayIndex'), 0.0) for _o in _opts], dtype=np.float32)
                _numbers = np.array([_v06d15_to_float((_o or {}).get('number'), 0.0) for _o in _opts], dtype=np.float32)
                _attack_ids = np.array([_v06d15_to_float((_o or {}).get('attackId'), 0.0) for _o in _opts], dtype=np.float32)
                _ability_ids = np.array([_v06d15_to_float((_o or {}).get('abilityId'), 0.0) for _o in _opts], dtype=np.float32)
                ...
                for _oi in range(_nopt):
                    ...
                    _X[_s_row + _oi, _V06D15_CARD_META_OFFSET:_V06D15_CARD_META_OFFSET + _V06D15_CARD_META_DIM] = _meta
                ...
                for _oi, _opt in enumerate(_opts):
                    _option_meta_rows.append({
                        ...
                        'option_type_name': _v06d15_name(_V06D15_OPTION_NAMES, int((_opt or {}).get('type', -1)) if isinstance(_opt, dict) else -1, 'OPT'),
                ...
                _decision_rows.append({
                    ...
                    'chosen_option_type_name': _v06d15_name(_V06D15_OPTION_NAMES, int(_chosen_opt.get('type', -1)), 'OPT'),
                    'rule_index': int(_rule_idx),
                    'rule_hit': int(_rule_idx == _act[0]),
                    'rank': _rank,
                    'rank_bucket': _v06d15_rank_bucket(_rank),
                    'won': bool(_won),
    ...
    _X = _X[:_cursor]
    _y = _y[:_cursor]
    _t1_extract = _v06d15_time.perf_counter()
    _extract_time_sec = _t1_extract - _t0_extract
    print(f'v0-06d15 extraction: {_extract_time_sec:.1f}s, {_cursor} option rows, {_decision_id} decisions')
    ...
    for _cc_obs, _cc_opts, _cc_nopt, _cc_start in _cross_check_buf:
        for _cc_oi, _cc_opt in enumerate(_cc_opts):
            _ref = _v06d15_option_features(_cc_obs, _cc_opt if isinstance(_cc_opt, dict) else {}, _cc_oi, _cc_nopt, _card_table)
    ...
    ARTIFACT_PATHS['v06d15_main_feature_matrix'] = str(_feat_npy_path)
    ARTIFACT_PATHS['v06d15_main_labels'] = str(_label_npy_path)

    _feature_spec = {
        'feature_dim': _V06D15_FEAT_DIM,
        'layout_version': 'v06d15',
        'card_id_hash_encoding': False,
        'card_metadata_encoding': True,
        'card_meta_dim': _V06D15_CARD_META_DIM,
        'card_meta_offset': _V06D15_CARD_META_OFFSET,
        'target': 'recorded replay MAIN option index for alakazam actor',
        'loss': 'per_decision_cross_entropy',
        'extraction_method': 'numpy_preallocated_vectorized_v06d15',
    ...
    ARTIFACT_PATHS['v06d15_main_feature_spec'] = write_json(OUTPUT_DIR / 'main_feature_spec.json', _feature_spec)
    ...
    _dataset_report = {
        ...
        'feature_dim': int(_V06D15_FEAT_DIM),
    ...
    ARTIFACT_PATHS['v06d15_main_dataset_report'] = write_json(OUTPUT_DIR / 'main_dataset_report.json', _dataset_report)
    ARTIFACT_PATHS['v06d15_main_decision_rows'] = safe_save_table(_decision_df, OUTPUT_DIR / 'main_decision_rows.parquet')
    ...
    # ------------------------------------------------------------------
    class _V06D15MainScorer(nn.Module):
    ...
    _model = _V06D15MainScorer(_V06D15_FEAT_DIM).to(_device)
    _param_count = int(sum(p.numel() for p in _model.parameters()))
    print(f'model: {_param_count} params, device={_device}')

    _max_epochs = int(os.environ.get('V06D15_MAIN_EPOCHS', '100'))
    _patience = int(os.environ.get('V06D15_MAIN_PATIENCE', '5'))
    _decision_batch = int(os.environ.get('V06D15_DECISION_BATCH', '256'))
    _lr = float(os.environ.get('V06D15_MAIN_LR', '0.002'))
    ...
    if _SKIP_TRAIN:
        ...
        _t0_gpu = _v06d15_time.perf_counter()
        try:
        ...
        for _ep in range(_max_epochs):
            ...
            for _st in range(0, _n_tr, _decision_batch):
                ...
                _xb_np = np.zeros((_B, _max_n, _V06D15_FEAT_DIM), dtype=np.float32)
                ...
                _logits = _model(_xb.view(_B * _max_n, _V06D15_FEAT_DIM)).view(_B, _max_n)
        ...
        _model.eval()
        _train_time_sec = _v06d15_time.perf_counter() - _t0_train

        # Save model
        torch.save(_model.state_dict(), _model_path)
    ARTIFACT_PATHS['v06d15_main_option_scorer'] = str(_model_path)

    # ------------------------------------------------------------------
    # Inference on full dataset (flat row-by-row for prediction table)
    # ------------------------------------------------------------------
    _t0_infer = _v06d15_time.perf_counter()
    ...
    _infer_time_sec = _v06d15_time.perf_counter() - _t0_infer
    ...
    ARTIFACT_PATHS['v06d15_main_holdout_predictions'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d15_main_eval_by_bucket'] = safe_save_table(_eval_df, OUTPUT_DIR / 'main_eval_by_bucket.parquet')

    # ------------------------------------------------------------------
    # Hybrid evaluation — keep v06d12 safe-family guard as a comparison baseline.
    # v06d15's main evidence is the conditioned ATTACK/END diagnostics below.
    # Threshold grid on valid; holdout only for promotion check.
    # ------------------------------------------------------------------
    _safe_override_types = [x.strip().upper() for x in
        os.environ.get('V06D15_SAFE_MAIN_OVERRIDE_TYPES', 'PLAY,ATTACH,EVOLVE,RETREAT').split(',') if x.strip()]
    _veto_types = ['ABILITY']  # only hard veto remaining
    _prob_grid = [float(x) for x in
        os.environ.get('V06D15_HYBRID_PROB_GRID', '0.35,0.45,0.55,0.65,0.75,0.85').split(',') if str(x).strip()]
    _margin_grid = [float(x) for x in
        os.environ.get('V06D15_HYBRID_MARGIN_GRID', '0.00,0.10,0.20,0.35,0.50').split(',') if str(x).strip()]
    ...
    ARTIFACT_PATHS['v06d15_main_hybrid_threshold_grid'] = safe_save_table(
    ...
    ARTIFACT_PATHS['v06d15_main_hybrid_eval_by_bucket'] = safe_save_table(
        _hybrid_bucket_df, OUTPUT_DIR / 'main_hybrid_eval_by_bucket.parquet')

    # ------------------------------------------------------------------
    # v06d15 diagnostics: condition evaluation on the rule-selected option family.
    # These reports are learning evidence only; they do not enable ATTACK/END runtime overrides.
    # ------------------------------------------------------------------
    _option_meta_reset = _option_meta_df.reset_index(drop=True)
    ...
    def _decision_option_slice(decision_row):
        _a = int(decision_row['option_start'])
        _b = int(decision_row['option_end'])
        _meta = _option_meta_reset.iloc[_a:_b].reset_index(drop=True)
        _scores = _all_logits[_a:_b]
        return _meta, _scores

    def _rule_type_conditioned_rows(df, split_name):
        _rows = []
        for _rtype, _g in df.groupby('rule_option_type_name', dropna=False):
            _rtype_s = str(_rtype)
            _cross = _g['model_top1_option_type_name'].astype(str).str.upper() != _g['rule_option_type_name'].astype(str).str.upper()
            _rows.append({
                'split': split_name,
                'rule_option_type_name': _rtype_s,
                'n': int(len(_g)),
                'rule_top1': float(_g['rule_hit'].mean()) if len(_g) else None,
                'model_top1': float(_g['model_top1_hit'].mean()) if len(_g) else None,
                'model_minus_rule_top1': float(_g['model_top1_hit'].mean() - _g['rule_hit'].mean()) if len(_g) else None,
                'cross_family_rate': float(_cross.mean()) if len(_g) else None,
                'model_top1_prob_mean': float(_g['model_top1_prob'].mean()) if len(_g) else None,
                'chosen_same_as_rule_family_rate': float(
                    (_g['chosen_option_type_name'].astype(str).str.upper() == _g['rule_option_type_name'].astype(str).str.upper()).mean()
                ) if len(_g) else None,
            })
        return _rows

    _conditioned_df = pd.DataFrame(
        _rule_type_conditioned_rows(_valid_pred, 'valid') +
        _rule_type_conditioned_rows(_hold, 'holdout')
    )
    ARTIFACT_PATHS['v06d15_rule_option_type_conditioned_report'] = safe_save_table(
        _conditioned_df, OUTPUT_DIR / 'rule_option_type_conditioned_report.parquet')

    def _attack_within_type_candidates(df, split_name):
        _rows = []
        _df = df[df['rule_option_type_name'].astype(str).str.upper() == 'ATTACK']
        for _, _pr in _df.iterrows():
            _dr = _decision_df.iloc[int(_pr['decision_id'])]
            _meta, _scores = _decision_option_slice(_dr)
            if _meta.empty:
                continue
            _attack_rel = [int(i) for i, _t in enumerate(_meta['option_type_name'].astype(str).str.upper().tolist()) if _t == 'ATTACK']
            if not _attack_rel:
                continue
            _attack_scores = _scores[_attack_rel]
            _attack_order = np.argsort(-_attack_scores)
            _model_rel = int(_attack_rel[int(_attack_order[0])])
            _second_score = float(_attack_scores[int(_attack_order[1])]) if len(_attack_order) > 1 else None
            _top_score = float(_attack_scores[int(_attack_order[0])])
            _chosen_idx = int(_dr['chosen_index'])
            _rule_idx = int(_dr['rule_index'])
            _chosen_is_attack = _chosen_idx in set(_attack_rel)
            _rows.append({
                'split': split_name,
                'decision_id': int(_pr['decision_id']),
                'n_options': int(_dr['n_options']),
                'n_attack_options': int(len(_attack_rel)),
                'chosen_is_attack': bool(_chosen_is_attack),
                'rule_index': int(_rule_idx),
                'model_attack_index': int(_model_rel),
                'rule_attack_hit': int(_rule_idx == _chosen_idx and _chosen_is_attack),
                'model_attack_hit': int(_model_rel == _chosen_idx and _chosen_is_attack),
                'model_attack_changed': int(_model_rel != _rule_idx),
                'model_attack_score': _top_score,
                'model_attack_prob': float(1.0 / (1.0 + np.exp(-_top_score))),
                'model_attack_margin': float(_top_score - _second_score) if _second_score is not None else 0.0,
                'global_model_type': str(_pr['model_top1_option_type_name']),
                'global_model_cross_family': bool(str(_pr['model_top1_option_type_name']).upper() != 'ATTACK'),
            })
        return pd.DataFrame(_rows)

    def _threshold_grid_for_attack(df, split_name):
        _rows = []
        if df.empty:
            return pd.DataFrame(_rows)
        for _pt in _prob_grid:
            for _mt in _margin_grid:
                _eligible = (
                    (df['n_attack_options'].astype(int) >= 2) &
                    (df['chosen_is_attack'].astype(bool)) &
                    (df['model_attack_changed'].astype(int) == 1) &
                    (df['model_attack_prob'].fillna(-999.0) >= _pt) &
                    (df['model_attack_margin'].fillna(-999.0) >= _mt)
                )
                _hybrid_hit = np.where(_eligible, df['model_attack_hit'].astype(int), df['rule_attack_hit'].astype(int))
                _benefit = _eligible & (df['rule_attack_hit'].astype(int) == 0) & (df['model_attack_hit'].astype(int) == 1)
                _harm = _eligible & (df['rule_attack_hit'].astype(int) == 1) & (df['model_attack_hit'].astype(int) == 0)
                _rule_t1 = float(df['rule_attack_hit'].mean())
                _hyb_t1 = float(np.mean(_hybrid_hit))
                _rows.append({
                    'split': split_name,
                    'prob_threshold': float(_pt),
                    'margin_threshold': float(_mt),
                    'n': int(len(df)),
                    'eligible_count': int(_eligible.sum()),
                    'eligible_rate': float(_eligible.mean()),
                    'rule_attack_top1': _rule_t1,
                    'attack_within_hybrid_top1': _hyb_t1,
                    'attack_within_delta': float(_hyb_t1 - _rule_t1),
                    'benefit_count': int(_benefit.sum()),
                    'harm_count': int(_harm.sum()),
                    'benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
                    'selection_ok': bool(int(_eligible.sum()) >= 10 and int(_benefit.sum()) > int(_harm.sum())),
                    'selection_score': float((_hyb_t1 - _rule_t1) + 0.001 * (int(_benefit.sum()) - int(_harm.sum()))),
                })
        return pd.DataFrame(_rows)

    _attack_valid = _attack_within_type_candidates(_valid_pred, 'valid')
    _attack_hold = _attack_within_type_candidates(_hold, 'holdout')
    _attack_valid_grid = _threshold_grid_for_attack(_attack_valid, 'valid')
    if not _attack_valid_grid.empty:
        _attack_valid_grid = _attack_valid_grid.sort_values(
            ['selection_ok', 'selection_score', 'attack_within_delta', 'eligible_rate'],
            ascending=[False, False, False, False],
        )
        _attack_sel = _json_safe(_attack_valid_grid.iloc[0].to_dict())
    else:
        _attack_sel = {'prob_threshold': 999.0, 'margin_threshold': 999.0, 'selection_ok': False}
    _attack_hold_grid = _threshold_grid_for_attack(_attack_hold, 'holdout')
    _attack_grid_all = pd.concat([_attack_valid_grid, _attack_hold_grid], ignore_index=True) if not _attack_hold_grid.empty else _attack_valid_grid
    ARTIFACT_PATHS['v06d15_attack_within_type_threshold_grid'] = safe_save_table(
        _attack_grid_all, OUTPUT_DIR / 'attack_within_type_threshold_grid.parquet')

    def _attack_summary(df, label, selected):
        if df.empty:
            return {'label': label, 'status': 'empty', 'n': 0}
        _pt = float(selected.get('prob_threshold', 999.0))
        _mt = float(selected.get('margin_threshold', 999.0))
        _eligible = (
            (df['n_attack_options'].astype(int) >= 2) &
            (df['chosen_is_attack'].astype(bool)) &
            (df['model_attack_changed'].astype(int) == 1) &
        ...
        _benefit = _eligible & (df['rule_attack_hit'].astype(int) == 0) & (df['model_attack_hit'].astype(int) == 1)
        _harm = _eligible & (df['rule_attack_hit'].astype(int) == 1) & (df['model_attack_hit'].astype(int) == 0)
        _multi = df['n_attack_options'].astype(int) >= 2
        return {
            'label': label,
            'status': 'ok',
            'n': int(len(df)),
            'multi_attack_decisions': int(_multi.sum()),
            'chosen_is_attack_rate': float(df['chosen_is_attack'].mean()),
            'global_cross_family_rate': float(df['global_model_cross_family'].mean()),
            'rule_attack_top1': float(df['rule_attack_hit'].mean()),
            'model_attack_top1': float(df['model_attack_hit'].mean()),
            'model_attack_minus_rule': float(df['model_attack_hit'].mean() - df['rule_attack_hit'].mean()),
            'selected_prob_threshold': _pt,
            'selected_margin_threshold': _mt,
            'selected_eligible_count': int(_eligible.sum()),
            'selected_eligible_rate': float(_eligible.mean()),
            'selected_attack_within_hybrid_top1': float(np.mean(_hybrid_hit)),
            'selected_attack_within_delta': float(np.mean(_hybrid_hit) - df['rule_attack_hit'].mean()),
            'selected_benefit_count': int(_benefit.sum()),
            'selected_harm_count': int(_harm.sum()),
            'selected_benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
        }

    _attack_report = {
        'status': 'ok' if not _attack_hold.empty else 'empty',
        'purpose': 'ATTACK within-type rerank only when rule selected ATTACK; no runtime action changes.',
        'valid_selected_threshold': _attack_sel,
        'valid_summary': _attack_summary(_attack_valid, 'valid', _attack_sel),
        'holdout_summary': _attack_summary(_attack_hold, 'holdout', _attack_sel),
    }
    ARTIFACT_PATHS['v06d15_attack_within_type_rerank_report'] = write_json(
        OUTPUT_DIR / 'attack_within_type_rerank_report.json', _json_safe(_attack_report))

    def _end_deferral_grid(df, split_name):
        _df = df[df['rule_option_type_name'].astype(str).str.upper() == 'END'].copy()
        _rows = []
        if _df.empty:
            return pd.DataFrame(_rows)
        _top_is_non_end = _df['model_top1_option_type_name'].astype(str).str.upper() != 'END'
        for _pt in _prob_grid:
            for _mt in _margin_grid:
                _eligible = (
                    _top_is_non_end &
                    (_df['model_top1_index'].astype(int) != _df['rule_index'].astype(int)) &
                    (_df['model_top1_prob'].fillna(-999.0) >= _pt) &
                    (_df['model_margin'].fillna(-999.0) >= _mt)
                )
                _hybrid_hit = np.where(_eligible, _df['model_top1_hit'].astype(int), _df['rule_hit'].astype(int))
                _benefit = _eligible & (_df['rule_hit'].astype(int) == 0) & (_df['model_top1_hit'].astype(int) == 1)
                _harm = _eligible & (_df['rule_hit'].astype(int) == 1) & (_df['model_top1_hit'].astype(int) == 0)
                _rule_t1 = float(_df['rule_hit'].mean())
                _hyb_t1 = float(np.mean(_hybrid_hit))
                _rows.append({
                    'split': split_name,
                    'prob_threshold': float(_pt),
                    ...
                    'eligible_count': int(_eligible.sum()),
                    'eligible_rate': float(_eligible.mean()),
                    'rule_end_top1': _rule_t1,
                    'end_deferral_hybrid_top1': _hyb_t1,
                    'end_deferral_delta': float(_hyb_t1 - _rule_t1),
                    'benefit_count': int(_benefit.sum()),
                    'harm_count': int(_harm.sum()),
                    'benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
                    'selection_ok': bool(int(_eligible.sum()) >= 10 and int(_benefit.sum()) > int(_harm.sum())),
    ...
    _end_valid_grid = _end_deferral_grid(_valid_pred, 'valid')
    if not _end_valid_grid.empty:
        _end_valid_grid = _end_valid_grid.sort_values(
            ['selection_ok', 'selection_score', 'end_deferral_delta', 'eligible_rate'],
            ascending=[False, False, False, False],
        )
        _end_sel = _json_safe(_end_valid_grid.iloc[0].to_dict())
    else:
        _end_sel = {'prob_threshold': 999.0, 'margin_threshold': 999.0, 'selection_ok': False}
    _end_hold_grid = _end_deferral_grid(_hold, 'holdout')
    _end_grid_all = pd.concat([_end_valid_grid, _end_hold_grid], ignore_index=True) if not _end_hold_grid.empty else _end_valid_grid
    ARTIFACT_PATHS['v06d15_end_deferral_threshold_grid'] = safe_save_table(
        _end_grid_all, OUTPUT_DIR / 'end_deferral_threshold_grid.parquet')

    def _end_summary(df, label, selected):
        _df = df[df['rule_option_type_name'].astype(str).str.upper() == 'END'].copy()
        if _df.empty:
            return {'label': label, 'status': 'empty', 'n': 0}
        _pt = float(selected.get('prob_threshold', 999.0))
        ...
        return {
            ...
            'status': 'ok',
            'n': int(len(_df)),
            'chosen_non_end_rate': float((_df['chosen_option_type_name'].astype(str).str.upper() != 'END').mean()),
            'model_top1_non_end_rate': float(_top_is_non_end.mean()),
            'rule_end_top1': float(_df['rule_hit'].mean()),
            'global_model_top1': float(_df['model_top1_hit'].mean()),
            'global_model_minus_rule': float(_df['model_top1_hit'].mean() - _df['rule_hit'].mean()),
            'selected_prob_threshold': _pt,
            'selected_margin_threshold': _mt,
            'selected_eligible_count': int(_eligible.sum()),
            'selected_eligible_rate': float(_eligible.mean()),
            'selected_end_deferral_hybrid_top1': float(np.mean(_hybrid_hit)),
            'selected_end_deferral_delta': float(np.mean(_hybrid_hit) - _df['rule_hit'].mean()),
            'selected_benefit_count': int(_benefit.sum()),
            'selected_harm_count': int(_harm.sum()),
            'selected_benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
        }
    ...
    _end_report = {
        'status': 'ok' if not _end_hold_grid.empty else 'empty',
        'purpose': 'END deferral diagnostic when rule selected END; no runtime action changes.',
        'valid_selected_threshold': _end_sel,
        'valid_summary': _end_summary(_valid_pred, 'valid', _end_sel),
        'holdout_summary': _end_summary(_hold, 'holdout', _end_sel),
    }
    ARTIFACT_PATHS['v06d15_end_deferral_report'] = write_json(
        OUTPUT_DIR / 'end_deferral_report.json', _json_safe(_end_report))

    _safe_end_deferral_types = ['PLAY', 'ATTACH', 'EVOLVE', 'RETREAT']

    def _end_deferral_safe_grid(df, split_name):
        _df = df[df['rule_option_type_name'].astype(str).str.upper() == 'END'].copy()
        _rows = []
        if _df.empty:
            return pd.DataFrame(_rows)
        _top_type = _df['model_top1_option_type_name'].astype(str).str.upper()
        _top_is_safe = _top_type.isin(_safe_end_deferral_types)
        for _pt in _prob_grid:
            for _mt in _margin_grid:
                _eligible = (
                    _top_is_safe &
                ...
                _benefit = _eligible & (_df['rule_hit'].astype(int) == 0) & (_df['model_top1_hit'].astype(int) == 1)
                _harm = _eligible & (_df['rule_hit'].astype(int) == 1) & (_df['model_top1_hit'].astype(int) == 0)
                _unsafe_top1 = _df['model_top1_option_type_name'].astype(str).str.upper().isin(['ABILITY', 'ATTACK', 'END', 'DISCARD'])
                _rule_t1 = float(_df['rule_hit'].mean())
                _hyb_t1 = float(np.mean(_hybrid_hit))
                _rows.append({
                    'split': split_name,
                    'prob_threshold': float(_pt),
                    'margin_threshold': float(_mt),
                    'safe_end_deferral_types': ','.join(_safe_end_deferral_types),
                    'n': int(len(_df)),
                    'eligible_count': int(_eligible.sum()),
                    'eligible_rate': float(_eligible.mean()),
                    'unsafe_top1_rate': float(_unsafe_top1.mean()),
                    'rule_end_top1': _rule_t1,
                    'safe_end_deferral_hybrid_top1': _hyb_t1,
                    'safe_end_deferral_delta': float(_hyb_t1 - _rule_t1),
                    'benefit_count': int(_benefit.sum()),
                    'harm_count': int(_harm.sum()),
                    'benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
                    'selection_ok': bool(int(_eligible.sum()) >= 10 and int(_benefit.sum()) > int(_harm.sum()) and _hyb_t1 >= _rule_t1),
                    'selection_score': float((_hyb_t1 - _rule_t1) + 0.001 * (int(_benefit.sum()) - int(_harm.sum()))),
                })
        return pd.DataFrame(_rows)

    _end_safe_valid_grid = _end_deferral_safe_grid(_valid_pred, 'valid')
    if not _end_safe_valid_grid.empty:
        _end_safe_valid_grid = _end_safe_valid_grid.sort_values(
            ['selection_ok', 'selection_score', 'safe_end_deferral_delta', 'eligible_rate'],
            ascending=[False, False, False, False],
        )
        _end_safe_sel = _json_safe(_end_safe_valid_grid.iloc[0].to_dict())
    else:
        _end_safe_sel = {'prob_threshold': 999.0, 'margin_threshold': 999.0, 'selection_ok': False}
    _end_safe_hold_grid = _end_deferral_safe_grid(_hold, 'holdout')
    _end_safe_grid_all = (
        pd.concat([_end_safe_valid_grid, _end_safe_hold_grid], ignore_index=True)
        if not _end_safe_hold_grid.empty else _end_safe_valid_grid
    )
    ARTIFACT_PATHS['v06d15_end_deferral_safe_threshold_grid'] = safe_save_table(
        _end_safe_grid_all, OUTPUT_DIR / 'end_deferral_safe_threshold_grid.parquet')

    def _end_safe_summary(df, label, selected):
        _df = df[df['rule_option_type_name'].astype(str).str.upper() == 'END'].copy()
        if _df.empty:
            return {'label': label, 'status': 'empty', 'n': 0}
        _pt = float(selected.get('prob_threshold', 999.0))
        ...
        _benefit = _eligible & (_df['rule_hit'].astype(int) == 0) & (_df['model_top1_hit'].astype(int) == 1)
        _harm = _eligible & (_df['rule_hit'].astype(int) == 1) & (_df['model_top1_hit'].astype(int) == 0)
        _selected_types = _df.loc[_eligible, 'model_top1_option_type_name'].astype(str).str.upper().value_counts().to_dict()
        return {
            'label': label,
            'status': 'ok',
            'n': int(len(_df)),
            'safe_end_deferral_types': list(_safe_end_deferral_types),
            'chosen_non_end_rate': float((_df['chosen_option_type_name'].astype(str).str.upper() != 'END').mean()),
            'model_top1_safe_rate': float(_top_is_safe.mean()),
            'rule_end_top1': float(_df['rule_hit'].mean()),
            'global_model_top1': float(_df['model_top1_hit'].mean()),
            'global_model_minus_rule': float(_df['model_top1_hit'].mean() - _df['rule_hit'].mean()),
            'selected_prob_threshold': _pt,
            ...
            'selected_eligible_count': int(_eligible.sum()),
            'selected_eligible_rate': float(_eligible.mean()),
            'selected_type_counts': {str(k): int(v) for k, v in _selected_types.items()},
            'selected_safe_end_deferral_hybrid_top1': float(np.mean(_hybrid_hit)),
            'selected_safe_end_deferral_delta': float(np.mean(_hybrid_hit) - _df['rule_hit'].mean()),
            'selected_benefit_count': int(_benefit.sum()),
            'selected_harm_count': int(_harm.sum()),
            'selected_benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
        }
    ...
    _end_safe_report = {
        'status': 'ok' if not _end_safe_hold_grid.empty else 'empty',
        'purpose': 'Safe END deferral runtime candidate: rule END may override only to PLAY/ATTACH/EVOLVE/RETREAT.',
        'valid_selected_threshold': _end_safe_sel,
        'valid_summary': _end_safe_summary(_valid_pred, 'valid', _end_safe_sel),
        'holdout_summary': _end_safe_summary(_hold, 'holdout', _end_safe_sel),
    }
    ARTIFACT_PATHS['v06d15_end_deferral_safe_report'] = write_json(
        OUTPUT_DIR / 'end_deferral_safe_report.json', _json_safe(_end_safe_report))

    # ------------------------------------------------------------------
    # v06d15: winner-conditioned rule/log disagreement diagnostics.
    # This is prioritization evidence, not causal proof that a winner-side action was optimal.
    # ------------------------------------------------------------------
    def _winner_disagreement_tables(pred_df):
        _d = pred_df[pred_df['split'].astype(str).isin(['valid', 'holdout'])].copy()
        if _d.empty:
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {'status': 'empty'}
        _d['rule_disagreement'] = _d['rule_hit'].astype(int) == 0
        _d['won_bool'] = _d['won'].astype(bool)
        _d['rule_family'] = _d['rule_option_type_name'].astype(str).str.upper()
        _d['chosen_family'] = _d['chosen_option_type_name'].astype(str).str.upper()
        _d['model_family'] = _d['model_top1_option_type_name'].astype(str).str.upper()
        _d['model_recovers_log'] = _d['model_top1_hit'].astype(int)
        _d['model_family_matches_log'] = (_d['model_family'] == _d['chosen_family']).astype(int)
        _d['high_rank'] = _d['rank_bucket'].astype(str).isin(['top10', 'top50', 'top200'])

        _dis = _d[_d['rule_disagreement']].copy()
        if _dis.empty:
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {'status': 'empty_disagreement'}

        _transition = (
            _dis.groupby(['split', 'won_bool', 'rule_family', 'chosen_family'], dropna=False)
            .agg(
                n=('decision_id', 'count'),
                model_recovery_rate=('model_recovers_log', 'mean'),
                model_family_match_rate=('model_family_matches_log', 'mean'),
                avg_model_top1_prob=('model_top1_prob', 'mean'),
                high_rank_count=('high_rank', 'sum'),
                avg_n_options=('n_options', 'mean'),
            )
            .reset_index()
            .sort_values(['split', 'won_bool', 'n'], ascending=[True, False, False])
        )

        _fam_base = (
            _dis.groupby(['split', 'chosen_family'], dropna=False)
            .agg(
                disagreement_count=('decision_id', 'count'),
                winner_count=('won_bool', 'sum'),
                model_recovery_rate=('model_recovers_log', 'mean'),
                model_family_match_rate=('model_family_matches_log', 'mean'),
                high_rank_count=('high_rank', 'sum'),
                avg_model_top1_prob=('model_top1_prob', 'mean'),
        ...
        _fam_base['loser_count'] = _fam_base['disagreement_count'] - _fam_base['winner_count']
        _fam_base['winner_share'] = _fam_base['winner_count'] / _fam_base['disagreement_count'].clip(lower=1)
        _fam_base['high_rank_rate'] = _fam_base['high_rank_count'] / _fam_base['disagreement_count'].clip(lower=1)
        _fam_base['candidate_score'] = (
            _fam_base['winner_count'].astype(float)
            * (0.5 + _fam_base['winner_share'].astype(float))
            * (0.25 + _fam_base['model_recovery_rate'].fillna(0.0).astype(float))
            * (0.25 + _fam_base['model_family_match_rate'].fillna(0.0).astype(float))
        )
        _family = _fam_base.sort_values(['split', 'candidate_score', 'winner_count'], ascending=[True, False, False])

        _rank = (
            _dis.groupby(['split', 'rank_bucket', 'won_bool', 'chosen_family'], dropna=False)
            .agg(
                n=('decision_id', 'count'),
                model_recovery_rate=('model_recovers_log', 'mean'),
                model_family_match_rate=('model_family_matches_log', 'mean'),
            ...
            .reset_index()
            .sort_values(['split', 'rank_bucket', 'won_bool', 'n'], ascending=[True, True, False, False])
        )

        def _summary_for_split(split):
            _s = _d[_d['split'].astype(str) == split]
            _sd = _dis[_dis['split'].astype(str) == split]
            _sf = _family[_family['split'].astype(str) == split].copy()
            _sf_non_end = _sf[~_sf['chosen_family'].isin(['END'])].copy()
            _sf_strong = _sf_non_end[
                (_sf_non_end['winner_count'] >= 50) &
                (_sf_non_end['model_family_match_rate'].fillna(0.0) >= 0.25)
            ].copy()
            _pick_df = _sf_strong if not _sf_strong.empty else _sf_non_end
            _pick = _pick_df.sort_values(['candidate_score', 'winner_count'], ascending=[False, False]).head(1)
            _top = _sf.sort_values(['candidate_score', 'winner_count'], ascending=[False, False]).head(8)
            return {
                'split': split,
                'n_decisions': int(len(_s)),
                'rule_disagreement_count': int(len(_sd)),
                'rule_disagreement_rate': float(len(_sd) / max(1, len(_s))),
                'winner_disagreement_count': int((_sd['won_bool']).sum()) if not _sd.empty else 0,
                'loser_disagreement_count': int((~_sd['won_bool']).sum()) if not _sd.empty else 0,
                'top_candidate_family_excluding_end': None if _pick.empty else str(_pick.iloc[0]['chosen_family']),
                'top_candidate_score_excluding_end': None if _pick.empty else float(_pick.iloc[0]['candidate_score']),
                'top_families': [
                    {
                        'chosen_family': str(r['chosen_family']),
                        'winner_count': int(r['winner_count']),
                        'loser_count': int(r['loser_count']),
                        'winner_share': float(r['winner_share']),
                        'model_recovery_rate': float(r['model_recovery_rate']),
                        'model_family_match_rate': float(r['model_family_match_rate']),
        ...
        _summary = {
            'status': 'ok',
            'purpose': 'Prioritize next action family from winner-conditioned rule/log disagreement; not causal proof.',
            'valid': _summary_for_split('valid'),
            'holdout': _summary_for_split('holdout'),
        }
        _hold_pick = _summary['holdout'].get('top_candidate_family_excluding_end')
        _valid_pick = _summary['valid'].get('top_candidate_family_excluding_end')
        _summary['recommended_next_family'] = _hold_pick
        _summary['recommendation_consistent_with_valid'] = bool(_hold_pick is not None and _hold_pick == _valid_pick)
        return _transition, _family, _rank, _summary

    _winner_transition_df, _winner_family_df, _winner_rank_df, _winner_disagreement_report = _winner_disagreement_tables(_pred_df)
    ARTIFACT_PATHS['v06d15_winner_rule_disagreement_transition_report'] = safe_save_table(
        _winner_transition_df, OUTPUT_DIR / 'winner_rule_disagreement_transition_report.parquet')
    ARTIFACT_PATHS['v06d15_winner_rule_disagreement_family_report'] = safe_save_table(
        _winner_family_df, OUTPUT_DIR / 'winner_rule_disagreement_family_report.parquet')
    ARTIFACT_PATHS['v06d15_winner_rule_disagreement_rank_report'] = safe_save_table(
        _winner_rank_df, OUTPUT_DIR / 'winner_rule_disagreement_rank_report.parquet')
    ARTIFACT_PATHS['v06d15_winner_rule_disagreement_report'] = write_json(
        OUTPUT_DIR / 'winner_rule_disagreement_report.json', _json_safe(_winner_disagreement_report))

    def _cross_family_report(df, label):
        _d = df[df['rule_option_type_name'].astype(str).str.upper().isin(['ATTACK', 'END'])].copy()
        if _d.empty:
            return {'label': label, 'status': 'empty', 'n': 0}
        _cross = _d['model_top1_option_type_name'].astype(str).str.upper() != _d['rule_option_type_name'].astype(str).str.upper()
        _harm = _cross & (_d['rule_hit'].astype(int) == 1) & (_d['model_top1_hit'].astype(int) == 0)
        _benefit = _cross & (_d['rule_hit'].astype(int) == 0) & (_d['model_top1_hit'].astype(int) == 1)
        _type_counts = (
            _d.assign(cross_family=_cross)
              .groupby(['rule_option_type_name', 'model_top1_option_type_name'], dropna=False)
        ...
        return {
            ...
            'status': 'ok',
            'n': int(len(_d)),
            'cross_family_count': int(_cross.sum()),
            'cross_family_rate': float(_cross.mean()),
            'cross_family_benefit_count': int(_benefit.sum()),
            'cross_family_harm_count': int(_harm.sum()),
            'cross_family_benefit_minus_harm': int(_benefit.sum() - _harm.sum()),
            'by_rule_model_top_pairs': [
                {'rule_option_type_name': str(k[0]), 'model_top1_option_type_name': str(k[1]), 'n': int(v)}
    ...
    _contamination_report = {
        'status': 'ok',
        'valid': _cross_family_report(_valid_pred, 'valid'),
        'holdout': _cross_family_report(_hold, 'holdout'),
    }
    ARTIFACT_PATHS['v06d15_cross_family_contamination_report'] = write_json(
        OUTPUT_DIR / 'cross_family_contamination_report.json', _json_safe(_contamination_report))
    ...
    MAIN_HYBRID_REPORT = {
        'status': 'ok',
        'version': RUN_PREFIX,
        'purpose': 'winner-conditioned rule-disagreement diagnostic using per-decision CE + card metadata features',
        'loss': 'per_decision_cross_entropy',
        'features': 'card_metadata_v06d15',
        'guard_scope': 'diagnostic only; no new action-changing runtime policy in v06d15',
        ...
        'rule_option_type_conditioned_report_rows': int(len(_conditioned_df)),
        'attack_within_type_report': _json_safe(_attack_report),
        'end_deferral_report': _json_safe(_end_report),
        'end_deferral_safe_report': _json_safe(_end_safe_report),
        'winner_rule_disagreement_report': _json_safe(_winner_disagreement_report),
        'cross_family_contamination_report': _json_safe(_contamination_report),
        'learning_candidates': {
            'attack_within_type_viable': bool(
                _attack_report.get('holdout_summary', {}).get('selected_benefit_minus_harm', 0) > 0 and
                _attack_report.get('holdout_summary', {}).get('selected_attack_within_delta', -1.0) >= 0.0
            ),
            'end_deferral_viable': bool(
                _end_report.get('holdout_summary', {}).get('selected_benefit_minus_harm', 0) > 0 and
                _end_report.get('holdout_summary', {}).get('selected_end_deferral_delta', -1.0) >= 0.0
            ),
            'safe_end_deferral_runtime_candidate': bool(
                _end_safe_report.get('holdout_summary', {}).get('selected_benefit_minus_harm', 0) > 0 and
                _end_safe_report.get('holdout_summary', {}).get('selected_safe_end_deferral_delta', -1.0) >= 0.0 and
                _end_safe_report.get('holdout_summary', {}).get('selected_eligible_count', 0) > 0
            ),
            'winner_disagreement_next_family': _winner_disagreement_report.get('recommended_next_family'),
            'winner_disagreement_consistent_with_valid': bool(
                _winner_disagreement_report.get('recommendation_consistent_with_valid', False)
            ),
        },
        'fresh_training': True,
    ...
    MAIN_LEARNING_REPORT = MAIN_HYBRID_REPORT
    ARTIFACT_PATHS['v06d15_main_model_report'] = write_json(OUTPUT_DIR / 'main_model_report.json', MAIN_HYBRID_REPORT)
    ARTIFACT_PATHS['v06d15_main_hybrid_report'] = write_json(OUTPUT_DIR / 'main_hybrid_report.json', MAIN_HYBRID_REPORT)

    print('v0-06d15 Part A status: ok')
    ...
    print('  attack within-type holdout:', _attack_report.get('holdout_summary', {}))
    print('  end deferral holdout:', _end_report.get('holdout_summary', {}))

except Exception as _v06d15_exc:
    MAIN_HYBRID_REPORT = {'status': 'error', 'error': repr(_v06d15_exc), 'traceback': traceback.format_exc(limit=8)}
    MAIN_LEARNING_REPORT = MAIN_HYBRID_REPORT
    ARTIFACT_PATHS['v06d15_main_model_report'] = write_json(OUTPUT_DIR / 'main_model_report.json', MAIN_HYBRID_REPORT)
    print('v0-06d15 Part A FAILED:', repr(_v06d15_exc))
    raise
