...
print('sensible_fwdsim vs generic_fwdsim (same deck, ISOLATES rollout: sensible vs greedy):')
for dn,dk in DECKS.items():
    w=g=0
    for i in range(N):
        a=(i%2==0)
        r=play((lambda o,d=dk:SF.sensible_fwdsim(o,d)),(lambda o,d=dk:G.generic_fwdsim(o,d)),dk,a)
        game.battle_finish()
        if r is None: continue
        g+=1
        if r==(0 if a else 1): w+=1
    print('  %-20s sensible_fwdsim_wr=%.3f (n=%d)'%(dn,w/max(1,g),g))
print('DONE_SF')
