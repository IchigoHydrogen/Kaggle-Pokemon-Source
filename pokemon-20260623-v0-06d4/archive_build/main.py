...
UNKNOWN0_POLICY_TABLE = {'12|13|14||safe||ko||late': '13', '12|13|14||safe||ko||mid': '13', '12|13|14||safe||no_ko||late': '13', '12|13|14|END||risk||ko||late': 'END', '12|13|14|END||safe||ko||early': 'END', '12|13|14|END||safe||ko||late': 'END', '12|13|14|END||safe||ko||mid': 'END', '12|13|14|END||safe||no_ko||late': 'END', '12|13|14|END||safe||no_ko||mid': 'END', '12|13|14|END|NO||safe||ko||late': 'END', '12|13|14|END|NO||safe||ko||mid': 'END', '12|13|14|END|NO||safe||no_ko||late': 'END', '12|13|14|END|NO|NUMBER||safe||ko||late': 'END', '12|13|14|END|NO|YES||risk||ko||late': 'END', '12|13|14|END|NO|YES||safe||ko||late': 'END', '12|13|14|END|NO|YES||safe||ko||mid': 'END', '12|13|14|END|NO|YES||safe||no_ko||late': 'END', '12|13|14|END|NUMBER||safe||ko||late': 'END', '12|13|14|END|NUMBER||safe||ko||mid': 'END', '12|13|14|END|NUMBER|YES||risk||ko||late': 'END', '12|13|14|END|NUMBER|YES||safe||ko||late': 'END', '12|13|14|END|YES||safe||ko||late': 'END', '12|13|14|END|YES||safe||ko||mid': 'END', '12|13|14|END|YES||safe||no_ko||late': 'END', '14|END||safe||ko||early': 'END', '14|END||safe||ko||late': 'END', '14|END||safe||ko||mid': 'END', '14|END||safe||no_ko||late': '14', '14|END|NO||safe||ko||late': 'END', '14|END|NO||safe||ko||mid': 'END', '14|END|NO|NUMBER|YES||safe||ko||late': 'END', '14|END|NO|YES||safe||ko||late': 'END', '14|END|NO|YES||safe||ko||mid': 'END', '14|END|NO|YES||safe||no_ko||late': 'END', '14|END|NUMBER|YES||safe||ko||late': 'END', '14|END|NUMBER|YES||safe||ko||mid': 'END', '14|END|YES||safe||ko||early': 'YES', '14|END|YES||safe||ko||late': 'END', '14|END|YES||safe||ko||mid': 'END', '14|END|YES||safe||no_ko||late': 'END', '12|13|14||*||*||*': '13', '12|13|14|END||*||*||*': 'END', '12|13|14|END|NO||*||*||*': 'END', '12|13|14|END|NO|YES||*||*||*': 'END', '12|13|14|END|NUMBER||*||*||*': 'END', '12|13|14|END|NUMBER|YES||*||*||*': 'END', '12|13|14|END|YES||*||*||*': 'END', '14|END||*||*||*': 'END', '14|END|NO||*||*||*': 'END', '14|END|NO|YES||*||*||*': 'END', '14|END|NUMBER|YES||*||*||*': 'END', '14|END|YES||*||*||*': 'END'}
...
try:
    ...
    _TORCH_POLICY_MODEL.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__), "pokemon-20260623-v0-06d4-torch_policy_state.pt"), map_location="cpu"))
...
def agent(obs_dict: dict) -> list[int]:
    ...
    op_prize_count = len(op_state.prize)
    ...
    early_game = state.turn <= 2
    mid_game = 3 <= state.turn <= 5
    late_game = state.turn >= 6
    ready_alakazam_in_play = any(
        p.id == Alakazam and any(ec.id in PSYCHIC_ENERGY_IDS for ec in p.energyCards)
        for _, p in my_field
    )
    has_energy_in_hand = (hand_counts[Basic_Psychic_Energy] + hand_counts[Telepath_Psychic_Energy] + hand_counts[Enriching_Energy]) > 0
    prize_pressure = my_prize_count <= 2 or op_prize_count <= 2 or can_win_this_turn
    need_attack_energy = not ready_alakazam_in_play and has_energy_in_hand and (field_counts[Alakazam] + field_counts[Kadabra] + field_counts[Abra] > 0)
    can_build_alakazam = (field_counts[Kadabra] >= 1 and hand_counts[Alakazam] >= 1) or (field_counts[Abra] >= 1 and hand_counts[Rare_Candy] >= 1 and hand_counts[Alakazam] >= 1)

    def tactical_card_bonus(card_id: int, base=0) -> int:
        bonus = int(base)
        if can_win_this_turn:
            if card_id == Boss_Orders and target_use_boss:
                bonus += 900
            if card_id == Enhanced_Hammer and target_hammer_needed > 0:
                bonus += 700
            if card_id in PSYCHIC_ENERGY_IDS and need_attack_energy:
                bonus += 500
        if setup_incomplete:
            if card_id == Alakazam and (field_counts[Kadabra] >= 1 or hand_counts[Rare_Candy] >= 1):
                bonus += 260
            elif card_id == Rare_Candy and field_counts[Abra] >= 1 and hand_counts[Alakazam] >= 1:
                bonus += 240
            elif card_id == Kadabra and field_counts[Abra] >= 1:
                bonus += 210
            elif card_id == Abra and abra_line_on_field < 3:
                bonus += 180
            elif card_id in (Buddy_Buddy_Poffin, Poke_Pad, Hilda, Dawn):
                bonus += 120 if not deckout_risk else -120
        if need_dunsparce_setup and card_id in (Dunsparce, Dudunsparce, Buddy_Buddy_Poffin):
            bonus += 110
        if late_game or prize_pressure:
            if card_id == Boss_Orders:
                bonus += 180
            elif card_id == Enhanced_Hammer:
                bonus += 120
            elif card_id == Nighttime_Mine and stadium_id != Nighttime_Mine:
                bonus += 100
        if deckout_risk and not can_win_this_turn:
            if card_id in (Hilda, Dawn, Poke_Pad, Buddy_Buddy_Poffin, Enriching_Energy, Dudunsparce):
                bonus -= 260
            if deckout_risk_strict and card_id not in (Boss_Orders, Enhanced_Hammer, Nighttime_Mine, Basic_Psychic_Energy, Telepath_Psychic_Energy, Rare_Candy, Alakazam, Kadabra):
                bonus -= 500
        return bonus
        ...
        if o.type == OptionType.NUMBER:
            ...
            if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                if o.playerIndex == my_index:
                    if card.id == Alakazam:
                        ...
                        if active_id != Alakazam and energy_count > 0:
                            score += 160
                        if target_can_kill:
                            score += 120
                ...
                score += tactical_card_bonus(card.id, 0)
                ...
                if card.id == Abra:
                    ...
                    score = 5
                    if hand_counts[Abra] > 0 or field_counts[Abra] > 0:
                        score += 4
                ...
                if card.id == Abra:
                    ...
                    if early_game and cur < 2:
                        score += 50
                elif card.id == Dunsparce:
                    score = 150 if dunsparce_line_on_field == 0 else 50
                    if early_game and abra_line_on_field >= 1:
                        score += 30

            elif context == SelectContext.TO_HAND:
                score = 200 - hand_counts.get(card.id, 0) * 50
                score += tactical_card_bonus(card.id)
                ...
                if card.id == Abra:
                    score = 100 + tactical_card_bonus(card.id, 0)
                    if bench_free <= 1 and abra_line_on_field >= 2:
                        score -= 140
                elif card.id == Dunsparce:
                    score = 80 + tactical_card_bonus(card.id, 0)
                    if bench_free <= 1 and dunsparce_line_on_field >= 1:
                        score -= 120
                elif card.id == Psyduck:
                    if op_has_duskull:
                        score = 60
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if card.id == Buddy_Buddy_Poffin:
                    ...
                    if target_hammer_needed > 0:
                        score = 6500 + tactical_card_bonus(card.id, 0)
                    else:
                        ...
                        if any_special:
                            score = (6200 if opponent_archetype in ('hop_control', 'lucario') else 5000) + tactical_card_bonus(card.id, 0)
                        elif opponent_archetype in ('hop_control', 'lucario') and state.turn >= 3:
                            score = 2200 + tactical_card_bonus(card.id, 0)
                        else:
                            score = -1
                    ...
                    if target_use_boss and target_can_kill:
                        score = (9000 if can_win_this_turn else 6200) + tactical_card_bonus(card.id, 0)
                    elif opponent_archetype == 'alakazam_mirror' and state.turn >= 4:
                        score = 2400 + tactical_card_bonus(card.id, 0)
                    ...
                    if safe_draws >= 2:
                        score = 3000 + tactical_card_bonus(card.id, 0)
                        if setup_incomplete and not state.supporterPlayed:
                            score += 900
                        elif late_game and not can_win_this_turn:
                            score -= 900
                    else:
                        score = -1
                    ...
                    if safe_draws >= 3:
                        score = 3100 + tactical_card_bonus(card.id, 0)
                        if setup_incomplete and not state.supporterPlayed:
                            score += 1100
                        elif late_game and not can_win_this_turn:
                            score -= 1100
                    ...
                    if stadium_id != Nighttime_Mine:
                        if opponent_archetype in ('hop_control', 'lucario'):
                            score = 15500 + tactical_card_bonus(card.id, 0)
                        elif opponent_archetype == 'alakazam_mirror' and state.turn >= 3:
                            score = 9800 + tactical_card_bonus(card.id, 0)
                        elif op_has_dragapult_line or target_can_kill:
                            score = 15000 + tactical_card_bonus(card.id, 0)
                        elif stadium_id != 0:
                            score = 7600 + tactical_card_bonus(card.id, 0)
                        elif state.turn >= 3:
                            score = 5300 + tactical_card_bonus(card.id, 0)
                        else:
                            score = -1
                    else:
                        score = -1
            ...
            if card.id == Air_Balloon:
                ...
                if need_retreat_energy and o.inPlayArea == AreaType.ACTIVE:
                    ...
                    score = 8000
                    if need_attack_energy or target_can_kill:
                        score += 450
            ...
            if card.id == Alakazam:
                ...
                if target_can_kill or setup_incomplete:
                    score += 500
                ...
                if safe_draws < 2:
                    ...
                    score += 80
                    if need_dunsparce_setup or need_dudunsparce_draw:
                        score += 120
            ...
            if o.attackId == ATTACK_POWERFUL_HAND:
                ...
                elif prize_pressure and max_damage >= max(1, op_active_hp - 40):
                    score += 700
    ...
    selected = _torch_policy_rerank(obs, selected, scores, context, safe_draws, can_win_this_turn, deckout_risk_strict)
